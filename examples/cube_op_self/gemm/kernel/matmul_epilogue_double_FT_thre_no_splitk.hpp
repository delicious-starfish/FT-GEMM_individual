#ifndef CATLASS_GEMM_KERNEL_MATMUL_DOUBLE_FT_THRESHOLD_NO_SPLITK_HPP_SELF
#define CATLASS_GEMM_KERNEL_MATMUL_DOUBLE_FT_THRESHOLD_NO_SPLITK_HPP_SELF
...
>
class MatmulFTDoubleBaseThreNoSplit {
public:
    ...
    using LayoutZ = typename BlockSumGemv::LayoutY;
    ...
    struct Params {
        ...
        Params(
            ...
            uint32_t SplitNnum_, bool outputThre_
    ...
    static size_t GetWorkspaceSize(const Arguments &args)
    {
        ...
        return sizeof(ElementY) * (args.problemGemmShape.m() + args.problemGemmShape.k() + splitNnum * args.problemGemmShape.m());
    }
    ...
    void Matmul_op(Params const &params){
        ...
        for (uint32_t loopIdx = AscendC::GetBlockIdx(); loopIdx < coreLoops; loopIdx += AscendC::GetBlockNum()) {
            ...
            int64_t gmOffsetYforFT = layoutYforFT.GetOffset(offsetYforFT) + params.problemGemmShape.m() + params.problemGemmShape.k();
    ...
    void SliceNSum_op(Params const &params)
    {
        ...
        gmOut.SetGlobalBuffer((__gm__ ElementSliceOut *)params.ptrZRow);
        uint32_t aiv_num = AscendC::GetBlockNum() * AscendC::GetTaskRation();
        for (uint32_t loop_id = 0; loop_id < loopnum; loop_id++) {
            ...
            if constexpr (std::is_same_v<LayoutYforFT, Catlass::layout::ColumnMajor>) {
                offset_matrix = loop_id * maxnPerBlock_round * params.problemSliceShape.m() + params.problemGemmShape.m() + params.problemGemmShape.k();
                offset_vector_out = (loop_id / split) * maxnPerBlock_round;

                m_actual = params.problemSliceShape.m();

            } else {
                offset_matrix = loop_id * maxnPerBlock_round + params.problemGemmShape.m() + params.problemGemmShape.k();
        ...
        GM_ADDR ptrOutputCOMP)
    {
        ...
        // uint32_t maxmPerBlock_round = RoundUp(ThreCalcUBTileShape::M, align);
        uint32_t maxmPerBlock_round = RoundUp(COMPUBTileShape::M, align);
        uint32_t tilemPerBlock_round = RoundUp(ThreCalcUBTileShape::M, align);

        uint32_t maxPerBlock_out_z = maxmPerBlock_round / 8;
        ...
        uint32_t offset_vector_out;
        uint32_t offset_vector_in = 0;
        uint32_t offset_vector_out_z = 0;
        ...
        for (uint32_t loop_id = 0; loop_id < loopnum; loop_id++) {
            ...
            if (loop_id % aiv_num != aiv_id)
                continue;
            uint32_t m_actual_total = ((int32_t)loop_id > (int32_t)(loopnum - split - 1))
                                        ? params.problemShape.m() - ((loop_id / split) * maxmPerBlock_round)
                                        : maxmPerBlock_round;
            
            uint32_t m_actual = (m_actual_total < tilemPerBlock_round) ? m_actual_total : tilemPerBlock_round;

            uint32_t n_actual = params.problemShape.n();
            uint32_t n_actual_total = params.problemShape.n();

            if constexpr (std::is_same_v<LayoutC, Catlass::layout::ColumnMajor>) {
                offset_matrix = (loop_id % split) * N_Split * params.problemShape.m() +
                                (loop_id / split) * maxmPerBlock_round;
                offset_vector_out = (loop_id / split) * maxmPerBlock_round;
                offset_vector_out_z = (loop_id / split) * maxPerBlock_out_z;
                ...
                offset_matrix = loop_id * maxmPerBlock_round * params.problemShape.n();
                offset_vector_out = loop_id * maxmPerBlock_round;
                offset_vector_out_z = loop_id * maxPerBlock_out_z;
            }
            Catlass::GemvCoord actualBlockShape = Catlass::GemvCoord{m_actual, n_actual};
            Catlass::GemvCoord actualBlockShapeTotal = Catlass::GemvCoord{m_actual_total, n_actual_total};
            ...
            */
            ...
            blockThreCalc(gmC[offset_matrix], params.layoutC,
                gmX[offset_vector_out], params.layoutThre,
                gmY[offset_vector_out], params.layoutThre,
                gmT[offset_vector_out], params.layoutThre,
                gmZ[offset_vector_out_z], layoutOutputZ,
