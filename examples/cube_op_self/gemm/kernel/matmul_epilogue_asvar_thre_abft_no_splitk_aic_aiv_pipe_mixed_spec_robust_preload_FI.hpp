#ifndef CATLASS_GEMM_KERNEL_MATMUL_BE_ABE_ON_AIC_ASVAR_THRESHOLD_ABFT_NO_SPLITK_HPP_SPEC_ROBUST_PRELOAD_FI
#define CATLASS_GEMM_KERNEL_MATMUL_BE_ABE_ON_AIC_ASVAR_THRESHOLD_ABFT_NO_SPLITK_HPP_SPEC_ROBUST_PRELOAD_FI
...
>
class MatmulAsVarABonAicNoSplitSpecRobustPreloadFI {
public:
    ...
    struct Params {
        ...
        Params(
            ...
            uint32_t KInjectUnit_, uint32_t inject_num_blk_,
            uint32_t inject_row_start_, uint32_t inject_col_, 
            uint32_t inject_bit_, GM_ADDR success_ptr_,
            GM_ADDR ptrFIIndex_, GM_ADDR ptrFIFlipData_
            ...
            ptrBE(ptrBE_), ptrBEforAIV(ptrBEforAIV_),
            ...
            SplitNnum(SplitNnum_), SplitMnum(SplitMnum_),
            SplitReduceM(SplitReduceM_), SplitReduceN(SplitReduceN_),
            outputThre(outputThre_), outputCE(outputCE_), 
            SplitKNum(SplitKNum_), KInjectUnit(KInjectUnit_),
            inject_num_blk(inject_num_blk_),
            inject_row_start(inject_row_start_), 
            inject_col(inject_col_), inject_bit(inject_bit_), 
            success_ptr(success_ptr_), ptrFIIndex(ptrFIIndex_),
            ptrFIFlipData(ptrFIFlipData_) { 
                for (int i = 0; i < 2; ++i) {
    ...
    static size_t GetWorkspaceSize(const Arguments &args)
    {
        ...
        return sizeof(ElementYforFT) * ((splitNnum + 1) * args.problemGemmShape.m() * args.SplitKNum);
    }
    ...
    static Params ToUnderlyingArguments(const Arguments &args, uint8_t *workspace)
    {
        ...
        uint32_t SplitMnum = ((args.problemGemmShape.m() + L1TileShape::M - 1) / L1TileShape::M);
        ...
        uint32_t inject_num_blk_aim = (k2 + args.KInjectUnit - 1) / args.KInjectUnit;

        uint32_t inject_num_blk = (inject_num_blk_aim <= args.inject_num_blk_limit) ? inject_num_blk_aim : args.inject_num_blk_limit;

        inject_num_blk = (inject_num_blk <= L1TileShape::M) ? inject_num_blk : L1TileShape::M;
        
        inject_num_blk = (inject_num_blk <= 1) ? 1 : inject_num_blk;

        uint32_t inject_bit_offset = (sizeof(ElementA) < sizeof(ElementYforFT)) ? 16 : 0;

        uint32_t inject_bit = args.inject_bit_raw + inject_bit_offset;
        ...
        */
        ...
        Params params{
            ...
            kn_sqrt_ratios, k_sqrt_n_ratios, n_scale_ratios,
            n_ratios, n_sqrt_ratios, n_square_ratios,
            SplitNnum, SplitMnum, 
            SplitReduceM, SplitReduceN,
            args.outputThre, args.outputCE, SplitKNum, args.KInjectUnit,
            inject_num_blk, args.inject_row_start,
            args.inject_col, inject_bit, args.success_ptr,
            args.ptrFIIndex, args.ptrFIFlipData
        };
    ...
    // Methods
    CATLASS_DEVICE
    MatmulAsVarABonAicNoSplitSpecRobustPreloadFI() {}
    ...
    void ABe_spec_on_AIC(Params const &params){
        ...
        gmVX.SetGlobalBuffer((__gm__ ElementXforFT *)params.ptrVXforA);
        ...
        for (uint32_t loopIdxTotal = AscendC::GetBlockIdx(); loopIdxTotal < coreLoopsTotal; loopIdxTotal += AscendC::GetBlockNum()) {
            ...
            int64_t gmOffsetVYforFTinBlock = blockCoord.m() * L1TileShapeforFT::M;
    ...
    void CE_split_op_fused(Params const &params, GM_ADDR ptrOutputCOMP)
    {
        ...
        AscendC::GlobalTensor<int32_t> gmFIIndex;
        gmFIIndex.SetGlobalBuffer((__gm__ int32_t *)params.ptrFIIndex);

        AscendC::GlobalTensor<ElementC> gmFIFlipData;
        gmFIFlipData.SetGlobalBuffer((__gm__ ElementC *)params.ptrFIFlipData);
        ...
        uint32_t inject_num_blk_align = RoundUp(params.inject_num_blk, UBAlignHelperOut::ALIGN);

        

        uint32_t inject_row_interval_upper = L1TileShape::M / params.inject_num_blk;
        ...
        uint32_t TileSliceNum = (UBBlockMRound +  UBTileMRound - 1) / UBTileMRound;
        uint32_t FI_blk_row_stride = TileSliceNum * inject_num_blk_align;
        uint32_t FI_total_row_stride = FI_blk_row_stride * params.SplitMnum;
        ...
        LayoutYforFT layoutFIInfo{params.SplitNnum, FI_total_row_stride};
        ...
        LayoutCOMPX layoutFIInfoVector{FI_total_row_stride};
            
        LayoutCOMPZ layoutOutputZ{total_output_elements};

        uint32_t inject_row_start_seed = params.inject_row_start + aicoreIndex;
        ...
        for (uint32_t loopIdx = aicoreIndex; loopIdx < coreLoops; loopIdx += aicoreNum) {
            ...
            uint32_t splitMIdx = blockCoord.m() * L1TileShape::M / L1TileShape::M;
            uint32_t Blk_Iter = (loopIdx - aicoreIndex) / aicoreNum;
            ...
            Catlass::MatrixCoord offsetFIInfo{splitNIdx, splitMIdx * FI_blk_row_stride};
            ...
            uint32_t inject_row_start = (inject_row_start_seed + Blk_Iter) % mActual;

            inject_row_interval_upper = mActual / params.inject_num_blk;

            if(inject_row_interval_upper < 1){
                inject_row_interval_upper = 1;
            }

            uint32_t inject_row_interval = inject_row_interval_upper;
            uint32_t inject_num_blk_actual = params.inject_num_blk;

            if(params.inject_num_blk < 2){
                inject_row_interval = mActual;
                inject_num_blk_actual = 1;
            }else{
                if(mActual < params.inject_num_blk){

                    inject_num_blk_actual = mActual;
                    inject_row_interval =  1;

                }else{

                    inject_num_blk_actual = params.inject_num_blk;

                    uint32_t inject_row_interval_raw = (inject_row_interval_upper + Blk_Iter * (inject_row_interval_upper - 1)) % inject_row_interval_upper;

                    if(inject_row_interval_raw < 1){
                        inject_row_interval = inject_row_interval_upper;
                    }else{
                        inject_row_interval = inject_row_interval_raw;
                    }
                }
            }
            ...
            int64_t gmOffsetFIInfo = layoutFIInfo.GetOffset(offsetFIInfo);

            int32_t block_ce_index = (int32_t)gmOffsetYforFT;
            ...
            Catlass::GemvCoord nextActualBlockShape = Catlass::GemvCoord{mActual, nActual};
            ...
            if(loopIdxNext < coreLoops){
                hasNextBlock = true;
                Catlass::GemmCoord blockCoordNext = matmulBlockScheduler.GetBlockCoord(loopIdxNext);
                Catlass::GemmCoord actualCoordNext = Catlass::GemmCoord({blockCoordNext.m() * L1TileShape::M, blockCoordNext.n() * L1TileShape::N, blockCoordNext.k() * L1TileShape::K});

                uint32_t splitNIdxNext = blockCoordNext.n() * L1TileShape::N / L1TileShape::N;

                // Compute initial location in logical coordinates for next Block
                Catlass::MatrixCoord offsetNextC{blockCoordNext.m() * L1TileShape::M, blockCoordNext.n() * L1TileShape::N};
                Catlass::MatrixCoord offsetNextYforFT{splitNIdxNext, blockCoordNext.m() * L1TileShape::M};
                Catlass::MatrixCoord offsetNextCOMPYforFT{splitNIdxNext, blockCoordNext.m() * L1TileShape::M};
                Catlass::MatrixCoord offsetNextCOMPXforFT{splitNIdxNext, blockCoordNext.m() * L1TileShape::M};
                Catlass::MatrixCoord offsetNextThreforFT{splitNIdxNext, blockCoordNext.m() * L1TileShape::M};

                uint32_t NextCOMPZRowOffset = blockCoordNext.m() * L1TileShape::M / 8;
                Catlass::MatrixCoord offsetNextCOMPZforFT{splitNIdxNext, NextCOMPZRowOffset};

                uint32_t mActualNext = UBBlockMRound;

                if(blockCoordNext.m() == loopsMN.row() -1) {
                    mActualNext = params.problemGemmShape.m() - blockCoordNext.m() * L1TileShape::M;
                }

                uint32_t nActualNext = ThreUBBlockNRound;

                if(blockCoordNext.n() == loopsMN.column() - 1){
                    nActualNext = params.problemCompShape.n() - blockCoordNext.n() * L1TileShape::N;
                }
                ...
                gmOffsetNextARed = blockCoordNext.m() * L1TileShape::M;
            ...
            */
            ...
            blockFTGemvAIV(
                ...
                inject_row_start, inject_num_blk_actual,
                inject_num_blk_align, inject_row_interval,
                params.inject_col, params.inject_bit, 
                block_ce_index, params.success_ptr,
                gmFIIndex[gmOffsetFIInfo], gmFIFlipData[gmOffsetFIInfo],
                layoutFIInfoVector,
                flagAicFinishStore);
    ...
    void ABE_reduce_for_FT_on_AIV(Params const &params)
    {
        ...
        for(uint32_t loopId = aivIndex; loopId < loopsNum; loopId += aivNum) {
            ...
            int64_t gmOffsetInAMean = OffsetAeSliceInit + loopIdN * UBTileNRound;
            int64_t gmOffsetOutAMean = loopIdN * UBTileNRound;
            ...
            */
        if(loopIdM == 0){
            blockSliceSum.add_ae_op(gmSliceABE[gmOffsetInSlice], layoutSliceIn,
                gmSliceAE[gmOffsetInAMean], layoutSliceAE,
                gmZRow[gmOffsetOutABE], layoutSliceOut,
                gmAMean[gmOffsetOutAMean], layoutSliceAE,
                actualBlockShape,
