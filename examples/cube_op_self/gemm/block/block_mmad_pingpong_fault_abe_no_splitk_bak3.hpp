...
#ifndef CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_HPP_FT_ABE_NO_SPLITK_THIRD
#define CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_HPP_FT_ABE_NO_SPLITK_THIRD
...
public:
    ...
    static constexpr uint32_t L1B_SIZE = L1TileShape::K * (L1TileShape::N+L1TileShapeforFT::N) * sizeof(ElementB);

    static constexpr uint32_t L1B_SIZE_for_B = L1TileShape::K * L1TileShape::N * sizeof(ElementB);
    static constexpr uint32_t L1B_SIZE_for_X = L1TileShapeforFT::K * L1TileShapeforFT::N * sizeof(ElementX);
    ...
    static_assert((L0B_TILE_SIZE * STAGES) <= L0B_SIZE_for_B, "L0TileShape exceeding the L0B space for holding B matrix!");
    static_assert((L0X_TILE_SIZE * STAGES) <= L0B_SIZE_for_X, "L0TileShape exceeding the L0B space for holding X matrix!");
    static_assert((L0B_TILE_SIZE * STAGES + L0X_TILE_SIZE * STAGES) <= L0B_SIZE, "L0TileShape exceeding the L0B space!");
    static_assert((L0B_SIZE_for_B + L0B_SIZE_for_X) <= L0B_SIZE, "L0TileShape for B and X exceeding the global L0B space");
    ...
    /// Construct
    CATLASS_DEVICE
    BlockMmadFTABeNoSplitK(Catlass::Arch::Resource<ArchTag> &resource, uint32_t l1BufAddrStart = 0)
    {   
        ...
        uint32_t l0XOffset = l0BOffset + L0B_PINGPONG_BUF_SIZE_for_B * STAGES;

        uint32_t l0COffset = 0;
        uint32_t l0COffsetforABe = l0COffset + L0C_SIZE_for_C;
        ...
        for(uint32_t i=0; i < STAGES; i++) {
            ...
            */
            ...
            l0BTensorList[i] = resource.l0BBuf.template GetBufferByByte<ElementB>(l0BOffset + L0B_PINGPONG_BUF_SIZE_for_B * i);
            // l0XTensorList[i] = resource.l0BBuf.template GetBufferByByte<ElementX>(l0XOffset + L0B_PINGPONG_BUF_SIZE_for_X * i);
            ...
            // AscendC::SetFlag<AscendC::HardEvent::M_MTE1>(l0XEventList[i]);
        }

        l0CTensor = resource.l0CBuf.template GetBufferByByte<ElementAccumulator>(l0COffset);
    ...
    ~BlockMmadFTABeNoSplitK()
    {
        for(uint32_t i=0; i < STAGES; i++){
            ...
            // AscendC::WaitFlag<AscendC::HardEvent::M_MTE1>(l0XEventList[i]);
        }
    ...
    {   
        // 对block上的矩阵进行对齐的补齐
        /*
        因为底层一次 MMAD 需要对齐为16 x 32 byte 进行计算，该规模的每个分块常称为FRACTAL
        其中32 byte 指的是内层连续轴上连续32byte，常被称为 C0
        所以需要对实际的shape进行padding操作，因此先将行列维度进行对齐。
        
        1. 对于A矩阵，M轴：
        a. 行/padding 行主序时，M 轴为外轴应当对齐于 16，即 C0_NUM_PER_FRACTAL 个元素
        b. 列/padding 列主序时，M 轴位于内轴，应当对齐于32 byte， 即ELE_NUM_PER_C0 个元素
        c. nZ 对齐时，因为分块内为列主序，所以M轴在每个分块内位于内轴，所以 M轴 应当对齐于32byte，即 ELE_NUM_PER_C0个元素
        d. zN 对齐时，因为分块内为行主序，所以M轴在每个分块内位于外轴，所以 M轴 应当对齐于16，即C0_NUM_PER_FRACTAL 个元素
        */

        /*
        2. 对于B矩阵, N轴
        a. 行/padding 行主序时，N 轴为内轴应当对齐于 32 Byte，即 ELE_NUM_PER_C0 个元素
        b. 列/padding 列主序时，N 轴位于外轴，应当对齐于 16，即 C0_NUM_PER_FRACTAL 个元素
        c. nZ 对齐时，因为分块内为列主序，所以N轴在每个分块内位于外轴，所以 N轴 应当对齐于 16，即 C0_NUM_PER_FRACTAL 个元素
        d. zN 对齐时，因为分块内为行主序，所以N轴在每个分块内位于内轴，所以 N轴 应当对齐于 32 byte，即 ELE_NUM_PER_C0 个元素
        */
        uint32_t mRound = RoundUp<L1AAlignHelper::M_ALIGNED>(actualShape.m());
        uint32_t nRound = RoundUp<L1BAlignHelper::N_ALIGNED>(actualShape.n());
        uint32_t nRoundTotal = nRound + L1TileShapeforFT::N;
        
        auto layoutAInL1 = LayoutAInL1::template MakeLayout<ElementA>(L1TileShape::M, L1TileShape::K);
        auto layoutBInL1Total = LayoutBInL1::template MakeLayout<ElementB>(L1TileShape::K, nRoundTotal);
        // auto layoutXInL1 = LayoutXInL1::template MakeLayout<ElementX>(L1TileShapeforFT::K, L1TileShapeforFT::N);
        auto layoutBInL1 = layoutBInL1Total.GetTileLayout(Catlass::MakeCoord(L1TileShape::K, nRound));
        auto layoutXInL1 = layoutBInL1Total.GetTileLayout(Catlass::MakeCoord(L1TileShape::K, L1TileShapeforFT::N));

        auto layoutInL0CTotal = LayoutCInL0::MakeLayoutInL0C(Catlass::MakeCoord(mRound, nRoundTotal));
        auto layoutInL0C = layoutInL0CTotal.GetTileLayout(Catlass::MakeCoord(mRound, nRound));
        auto layoutInL0CforABe = layoutInL0CTotal.GetTileLayout(Catlass::MakeCoord(mRound, L1TileShapeforFT::N));
        ...
        Catlass::MatrixCoord l1BOffsetNow{uint32_t(0), uint32_t(0)};

        auto l1BTile = l1BTensor[layoutBInL1Total.GetOffset(l1BOffsetNow)];
                        
        // Locate the current tile of matrix X on L1
        Catlass::MatrixCoord l1XOffsetNow{uint32_t(0), nRound};
                        
        auto l1XTile = l1BTensor[layoutBInL1Total.GetOffset(l1XOffsetNow)];

        AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(l1BEventList[l1ListId]);
        auto layoutTileB = layoutB.GetTileLayout(Catlass::MakeCoord(kActual,actualShape.n()));
        copyGmToL1B(l1BTile, gmB, layoutBInL1, layoutTileB);
        
        auto layoutTileX = layoutX.GetTileLayout(Catlass::MakeCoord(kActual,uint32_t(1)));
        copyGmToL1X(l1XTile, gmX, layoutXInL1, layoutTileX);
        ...
        for(uint32_t kLoopIdx=0; kLoopIdx < kTileCount; kLoopIdx++)
        {
            ...
            if (kLoopIdx < kTileCount - 1){
                ...
                Catlass::MatrixCoord gmTileXOffset{kLoopIdxNext * L1TileShape::K, 0};
                ...
                auto l1BTileNext = l1BTensor[layoutBInL1Total.GetOffset(l1BOffsetNow)];
                        
                // Locate the current tile of matrix X on L1                        
                auto l1XTileNext = l1BTensor[layoutBInL1Total.GetOffset(l1XOffsetNow)];
                ...
                copyGmToL1B(l1BTileNext, gmTileB, layoutBInL1, layoutTileB);

                layoutTileX = layoutX.GetTileLayout(Catlass::MakeCoord(kActualNext, uint32_t(1)));
                copyGmToL1X(l1XTileNext, gmTileX, layoutXInL1, layoutTileX);
            ...
            for(int mPartIdx=0; mPartIdx < mPartLoop; mPartIdx++){
                ...
                for(int kPartIdx=0; kPartIdx < kPartLoop; kPartIdx++){
                    ...
                    for(int nPartIdx = 0; nPartIdx < nPartLoop; nPartIdx++){
                        ...
                        uint32_t nPartActualTotal = nPartActual + L0TileShapeforFT::N;
                        
                        // Locate the current tile for B on L0B
                        auto l0BTile = l0BTensorList[l0BListId];
                        LayoutBInL0 layoutBInL0 = 
                            LayoutBInL0::template MakeLayout<ElementB>(kPartActual, nPartActualTotal);
                        
                        // Locate the current tile of matrix B on L1
                        Catlass::MatrixCoord l1BOffset{kPartIdx * L0TileShape::K, 
                            nPartIdx * nRoundTotal};

                        auto l1BTile = l1BTensor[layoutBInL1Total.GetOffset(l1BOffset)];
                        ...
                        // Load current tile from L1 to L0B for B
                        copyL1ToL0B(l0BTile, l1BTile, layoutBInL0, layoutBInL1Total);
                        ...
                        // Locate the current tile on L0C for C
                        Catlass::MatrixCoord l0COffset{mPartIdx * L0TileShape::M, nPartIdx * nRoundTotal};
                        // 获取当前局部输出
                        ...
                        // Perform calculation operations for C matrix
                        tileMmad(l0CTile, l0ATile, l0BTile, mPartActual, 
                            nPartActualTotal, kPartActual, initC, unitFlag);
        ...
        LayoutY layoutBlockforABe = layoutY.GetTileLayout(Catlass::MakeCoord(actualShape.m(),uint32_t(1)));

        if constexpr (!ENABLE_UNIT_FLAG) {
            // 标记开始写入cGM 数据
            AscendC::SetFlag<AscendC::HardEvent::M_FIX>(MMAD_EVENT_ID0);

            Catlass::MatrixCoord l0COffsetforB{uint32_t(0), uint32_t(0)};
            auto l0CTile = l0CTensor[layoutInL0CTotal.GetOffset(l0COffsetforB)];
            ...
            // 写入数据
            copyL0CToGm(gmC, l0CTile, layoutBlock, layoutInL0C);
            // 写入数据
            ...
            copyL0CToGm(gmC, l0CTile, layoutBlock, layoutInL0C, 0b11);
