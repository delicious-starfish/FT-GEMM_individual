...
#ifndef CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_HPP_FT_NO_SPLITK_SELF
#define CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_HPP_FT_NO_SPLITK_SELF
...
public:
    ...
    */
    ...
    static constexpr uint32_t L0C_TILE_NUM = (L1TileShape::M + L0TileShapeforFT::M - 1) / L0TileShapeforFT::M;
    static constexpr uint32_t L0C_TILE_STAGES = (L0C_TILE_NUM <= STAGES) ? L0C_TILE_NUM : STAGES;
    
    static constexpr uint32_t L0C_TILE_SIZE = L0C_SIZE / L0C_TILE_NUM / sizeof(ElementAccumulator);

    // L0C_SIZE / L0C_TILE_SIZE / sizeof(ElementAccumulator);
    // // static constexpr uint32_t L0C_TILE_NUM = STAGES;
    // // static constexpr uint32_t L0C_TILE_SIZE = L0C_SIZE / L0C_TILE_NUM / sizeof(ElementAccumulator);

    // static constexpr uint32_t L0C_SIZE = ArchTag::L0C_SIZE;
    // static constexpr uint32_t L0C_TILE_SIZE = L0TileShapeforFT::M * L0TileShapeforFT::N;
    
    // static constexpr uint32_t L0C_TILE_NUM = (L0C_TILE_NUM_RAW <= 8) ? L0C_TILE_NUM_RAW : 8;
    
    // L0C_SIZE / L0C_TILE_SIZE / sizeof(ElementAccumulator)
    // static constexpr uint32_t L0C_TILE_NUM_RAW = L0C_SIZE / L0C_TILE_SIZE / sizeof(ElementAccumulator);
    // static constexpr uint32_t L0C_TILE_NUM = (L0C_TILE_NUM_RAW <= 8) ? L0C_TILE_NUM_RAW : 8;
    ...
    static_assert((L1A_SIZE * STAGES + L1B_SIZE * STAGES + L1FT_SIZE) <= ArchTag::L1_SIZE, 
    ...
    CATLASS_DEVICE
    BlockMmadFTNOSPLIT(Catlass::Arch::Resource<ArchTag> &resource, uint32_t l1BufAddrStart = 0)
    {   
        ...
        for(uint32_t i=0; i < STAGES; i++) {
            ...
            */
            ...
            //srcTensor_x_rep = srcTensor_x.template ReinterpretCast<ElementWIn>();
            // l0FTTensorList[i] = resource.l0BBuf.template GetBufferByByte<ElementX>(L0B_PINGPONG_BUF_SIZE * i);
        ...
        l0FTEvent = STAGES * 2 + 1;
        AscendC::SetFlag<AscendC::HardEvent::M_MTE1>(l0FTEvent);

        l0CTensor = resource.l0CBuf.template GetBufferByByte<ElementAccumulator>(0);
        l1FTTensor = resource.l1Buf.template GetBufferByByte<ElementX>(l1FTOffset);
        ...
        // AscendC::SetFlag<AscendC::HardEvent::MTE1_MTE2>(l1FTEvent);
        AscendC::SetFlag<AscendC::HardEvent::FIX_MTE2>(l1FTEvent);
        AscendC::SetFlag<AscendC::HardEvent::M_MTE1>(l1FTEvent);

        AscendC::SetFlag<AscendC::HardEvent::M_FIX>(l1FTEvent);
        AscendC::SetFlag<AscendC::HardEvent::FIX_M>(l1FTEvent);
        ...
        AscendC::SetFlag<AscendC::HardEvent::MTE2_MTE1>(l1FTEvent);
    ...
    ~BlockMmadFTNOSPLIT()
    {
        ...
        AscendC::WaitFlag<AscendC::HardEvent::M_MTE1>(l0FTEvent);
        AscendC::WaitFlag<AscendC::HardEvent::FIX_M>(MMAD_EVENT_ID0);
        
        // AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(l1FTEvent);
        AscendC::WaitFlag<AscendC::HardEvent::FIX_MTE2>(l1FTEvent);
        AscendC::WaitFlag<AscendC::HardEvent::M_MTE1>(l1FTEvent);

        AscendC::WaitFlag<AscendC::HardEvent::M_FIX>(l1FTEvent);
        AscendC::WaitFlag<AscendC::HardEvent::FIX_M>(l1FTEvent);

        AscendC::WaitFlag<AscendC::HardEvent::MTE2_MTE1>(l1FTEvent);
        ...
        Catlass::GemmCoord const& actualShape)
    {
        ...
        AscendC::SetFlag<AscendC::HardEvent::MTE2_MTE1>(l0FTEvent);
        AscendC::WaitFlag<AscendC::HardEvent::MTE2_MTE1>(l0FTEvent);
        ...
        */
        ...
        AscendC::SetFlag<AscendC::HardEvent::MTE1_M>(l0FTEvent);
        ...
        Catlass::GemmCoord const& actualShape, Catlass::GemmCoord const& actualCoord)
    {
        ...
        auto layoutInL0C = LayoutYInL0::MakeLayoutInL0C(Catlass::MatrixCoord(L1XAlignHelper::M_ALIGNED, L0TileShapeforFT::M));
        ...
        uint32_t mPartLoop = CeilDiv<L0TileShapeforFT::M>(actualShape.m());
        ...
        AscendC::WaitFlag<AscendC::HardEvent::MTE1_M>(l0FTEvent);
        AscendC::WaitFlag<AscendC::HardEvent::M_MTE1>(MMAD_EVENT_ID0);
        AscendC::WaitFlag<AscendC::HardEvent::FIX_M>((int32_t)(MMAD_EVENT_ID0));
        
        
        for(uint32_t mPartIdx=0; mPartIdx < mPartLoop; mPartIdx++){
            uint32_t shuffleKIdx = mPartIdx;
            uint32_t singleIdx = (mPartIdx % L0C_TILE_NUM);
            uint32_t L1CMOffset = mPartIdx * L0TileShapeforFT::M;

            uint32_t nRound = RoundUp<L1BAlignHelper::N_ALIGNED>(nActual);
            uint32_t nPartLoop = CeilDiv<L0TileShapeforFT::N>(nActual);

            uint32_t mPartActual =
                    (mPartIdx < mPartLoop - 1) ? L0TileShapeforFT::M : (mActual - mPartIdx * L0TileShapeforFT::M);
            ...
            for (uint32_t nPartIdx = 0; nPartIdx < nPartLoop; nPartIdx++) {
                ...
                AscendC::SetFlag<AscendC::HardEvent::MTE1_M>(l0CforFTEventList[(uint32_t)(singleIdx % L0C_TILE_STAGES)]);

                auto l0CTile = l0CTensor[singleIdx * L0C_TILE_SIZE];

                // If the current tile is the first tile on the k axis, the accumulator needs to be reset to 0
                bool initC = (nPartIdx == 0);

                AscendC::WaitFlag<AscendC::HardEvent::MTE1_M>(l0BEventList[l0BListId]);
                AscendC::WaitFlag<AscendC::HardEvent::MTE1_M>(l0CforFTEventList[(uint32_t)(singleIdx % L0C_TILE_STAGES)]);
                // // L0TileShapeforFT::M
            ...
            // startMCoord + 
            copyL0CToGmforFT(gmBlockY[singleIdx * L0TileShapeforFT::M], l0CTile, layoutBlock, layoutInL0C);
        ...
        Catlass::GemmCoord const& actualShape, Catlass::GemmCoord const& actualCoord)
    {
        ...
        auto layoutxInL0 =
                    LayoutXInL0::template MakeLayout<ElementX>(L1XAlignHelper::M_ALIGNED, L1TileShape::N);
        ...
        uint32_t singleIdx = l1ListId % L0C_TILE_NUM;
        ...
        for (uint32_t nLoopIdx = 0; nLoopIdx < nTileCount; nLoopIdx++) {
            ...
            for (uint32_t nPartIdx = 0; nPartIdx < nPartLoop; nPartIdx++) {
                ...
                Catlass::MatrixCoord l0xOffset{0, nPartIdx * L0TileShapeforFT::N};
                auto l0XTile = l0XTensor[layoutxInL0.GetOffset(l0xOffset)];
        ...
        // copy block out
        // actualShape.m()
        LayoutY layoutBlock = layoutY.GetTileLayout(Catlass::MakeCoord(uint32_t(1), mActual));

        AscendC::SetFlag<AscendC::HardEvent::M_FIX>(l0CforFTEventList[(uint32_t)(singleIdx % L0C_TILE_STAGES)]);
        AscendC::WaitFlag<AscendC::HardEvent::M_FIX>(l0CforFTEventList[(uint32_t)(singleIdx % L0C_TILE_STAGES)]);

        copyL0CToGmforFT(gmBlockY, l0CTile, layoutBlock, layoutInL0C);
        // AscendC::SetFlag<AscendC::HardEvent::FIX_M>((int32_t)(singleIdx % L0C_TILE_STAGES));
        AscendC::SetFlag<AscendC::HardEvent::FIX_M>(l0CforFTEventList[(uint32_t)(singleIdx % L0C_TILE_STAGES)]);
        AscendC::SetFlag<AscendC::HardEvent::FIX_M>(MMAD_EVENT_ID0);
        AscendC::SetFlag<AscendC::HardEvent::M_MTE1>(l0FTEvent);
    ...
    {   
        ...
        */
        ...
        if constexpr (!ENABLE_UNIT_FLAG) {
            ...
            AscendC::WaitFlag<AscendC::HardEvent::M_FIX>(l1FTEvent);
            copyL0CToL1forFT(l1FTTensor, l0CTensor, layoutCInL1forFT, layoutInL0C);
            // 标记数据写入 cGM 已经完成
            
            AscendC::SetFlag<AscendC::HardEvent::FIX_M>(MMAD_EVENT_ID0);
        } else {
            copyL0CToGm(gmC, l0CTensor, layoutBlock, layoutInL0C, 0b11);
            copyL0CToL1forFT(l1FTTensor, l0CTensor, layoutCInL1forFT, layoutInL0C, 0b11, false);
        }
        ...
        // RowSum2(gmBlockY,layoutY,actualShape,actualCoord);
        RowSum(gmBlockY,layoutY,actualShape,actualCoord);
...
protected:
    ...
    AscendC::LocalTensor<ElementX> l1FTTensor;
