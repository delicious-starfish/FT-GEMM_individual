...
#ifndef CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_HPP_FT_ABE_NO_SPLITK_BAK_SEC
#define CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_HPP_FT_ABE_NO_SPLITK_BAK_SEC
...
public:
    ...
    {   
        ...
        for(uint32_t i=0; i < STAGES; i++) {
            ...
            */
            ...
            l0BTensorList[i] = resource.l0BBuf.template GetBufferByByte<ElementB>(l0BOffset + L0B_PINGPONG_BUF_SIZE * i);
    ...
    {   
        ...
        */
        ...
        auto layoutInL0CTotal = LayoutCInL0::MakeLayoutInL0C(Catlass::MakeCoord(mRound, (nRound + L1TileShapeforFT::N)));
        ...
        // LayoutCInL0::MakeLayoutInL0C(Catlass::MakeCoord(mRound, nRound));
        // auto layoutInL0CforABe = LayoutCInL0::MakeLayoutInL0C(Catlass::MakeCoord(mRound, L1TileShapeforFT::N));
        ...
        // load first matrix X tile from GM to L1
        ...
        for(uint32_t kLoopIdx=0; kLoopIdx < kTileCount; kLoopIdx++)
        {
            ...
            if (kLoopIdx < kTileCount - 1){
                ...
                // load next matrix X tile from GM to L1
                AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(l1XEventList[l1ListIdNext]);
                // layoutX.GetTileLayout(Catlass::MakeCoord(kActual,uint32_t(1)));
                ...
                for(int kPartIdx=0; kPartIdx < kPartLoop; kPartIdx++){
                    ...
                    for(int nPartIdx = 0; nPartIdx < nPartLoop; nPartIdx++){
                        ...
                        LayoutBInL0 layoutBInL0Total = 
                            LayoutBInL0::template MakeLayout<ElementB>(kPartActual, (nPartActual + L0TileShapeforFT::N));
                        
                        /*
                        CATLASS_HOST_DEVICE
                        nZ GetTileLayout(MatrixCoord const &tileOriShape) const
                        */
                        // layoutC.GetTileLayout(actualShape.GetCoordMN());
                        LayoutBInL0 layoutBInL0 = layoutBInL0Total.GetTileLayout(Catlass::MakeCoord(kPartActual, nPartActual));
                            // MakeLayout<ElementB>();
                        
                        LayoutBInL0 layoutXInL0 = layoutBInL0Total.GetTileLayout(Catlass::MakeCoord(kPartActual, L0TileShapeforFT::N));
                            // LayoutXInL0::template MakeLayout<ElementX>(kPartActual, L0TileShapeforFT::N);
                        

                        // Locate the current tile of matrix B on L1
                        Catlass::MatrixCoord l0BOffset{0, 
                            nPartIdx * (nPartActual + L0TileShapeforFT::N)};

                        auto l0BTile = l0BTensor[layoutBInL0Total.GetOffset(l0BOffset)];
                        
                        // Locate the current tile of matrix X on L1
                        Catlass::MatrixCoord l0XOffset{0, 
                            nPartIdx * (nPartActual + L0TileShapeforFT::N)+nPartActual};
                        
                        auto l0XTile = l0BTensor[layoutBInL0Total.GetOffset(l0XOffset)];
                        
                        // Locate the current tile for X on L0B
                        // auto l0XTile = l0XTensorList[l0BListId];
                        ...
                        copyL1ToL0X(l0XTile, l1XTile, layoutXInL0, layoutXInL1);
                        ...
                        // // Wait for mmad of ABe finished
                        // // 等待当前阶段L0 数据消费完成，即相应 MMAD for ABe 计算完成
                        // AscendC::WaitFlag<AscendC::HardEvent::M_MTE1>(l0XEventList[l0BListId]);
                        // // Load current tile from L1 to L0B for X
                        ...
                        // // Locate the current tile on L0C for C
                        // Catlass::MatrixCoord l0COffsetforABe{mPartIdx * L0TileShape::M, nPartIdx * L0TileShapeforFT::N};
                        // // 获取当前局部输出
                        ...
                        tileMmad(l0CTile, l0ATile, l0BTensor, mPartActual, 
                            (nPartActual+L0TileShapeforFT::N), kPartActual, initC, unitFlag);
                        
                        // AscendC::WaitFlag<AscendC::HardEvent::MTE1_M>(MMAD_ABE_EVENT_ID1);

                        // // Perform calculation operations for C matrix
                        // tileMmadforABe(l0CTileforABe, l0ATile, l0XTile, mPartActual, 
                        //     L0TileShapeforFT::N, kPartActual, initC, unitFlag);
