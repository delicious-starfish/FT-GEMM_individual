...
#ifndef CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_HPP_FT_ABE_NO_SPLITK_SELF_AUG_ROBUST
#define CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_HPP_FT_ABE_NO_SPLITK_SELF_AUG_ROBUST
...
public:
    ...
    using CopyGmToL1VX = typename TileCopyFTABonAicAuged_::CopyGmToL1VX;
    using CopyGmToL1VXCol = typename TileCopyFTABonAicAuged_::CopyGmToL1VXCol;
    ...
    static constexpr uint32_t L1VX_SIZE = L1TileShapeforFT::K * 16 * sizeof(ElementX);
    ...
    static constexpr uint32_t L0B_SIZE_FOR_VX = L0TileShapeforFT::K * 16 * sizeof(ElementX) * STAGES;
    ...
    static constexpr uint32_t L0B_PINGPONG_BUF_SIZE_FOR_X = (L0B_SIZE_FOR_X + L0B_SIZE_FOR_VX) / STAGES;
    ...
    static constexpr uint32_t L0VX_TILE_SIZE = L0TileShapeforFT::K * L1XAlignHelper::M_ALIGNED * sizeof(ElementX);
    ...
    static constexpr uint32_t L0C_SIZE_for_AE = L0TileShape::M * L1XAlignHelper::M_ALIGNED * sizeof(ElementAccumulator);
    ...
    static_assert((L0VX_TILE_SIZE * STAGES) <= L0B_SIZE_FOR_VX, "L0TileShape exceeding the space of L0B for X of AE!");

    static_assert((L0B_TILE_SIZE * STAGES + L0X_TILE_SIZE * STAGES + L0VX_TILE_SIZE * STAGES) <= L0B_SIZE, "L0TileShape exceeding the space of L0B!");
    static_assert((L0B_SIZE_FOR_B + L0B_SIZE_FOR_X + L0B_SIZE_FOR_VX) <= L0B_SIZE, "space for B and X exceeding the L0B space!");

    static_assert((L0C_SIZE_for_C + L0C_SIZE_for_ABE + L0C_SIZE_for_AE) <= L0C_SIZE, "L0TileShape for C, ABe and Ae exceeding the global L0C space");
    ...
    /// Construct
    CATLASS_DEVICE
    BlockMmadFTABeAugedNoSplitKRobust(Catlass::Arch::Resource<ArchTag> &resource, uint32_t l1BufAddrStart = 0)
    {   
        ...
        uint32_t l0COffsetforAe = l0COffsetforC + L0C_SIZE_for_C + L0C_SIZE_for_ABE;
    ...
    /// Destructor
    CATLASS_DEVICE
    ~BlockMmadFTABeAugedNoSplitKRobust()
    {
        for(uint32_t i=0; i < STAGES; i++){
            // 等待相关内存事件完成后再结束运行
    ...
    */
    ...
    void add_ae_op(
        ...
        AscendC::GlobalTensor<ElementC> const & gmC, LayoutC const &layoutC,
        AscendC::GlobalTensor<ElementX> const & gmX, LayoutXCol const &layoutXCol,
        AscendC::GlobalTensor<ElementX> const & gmVX, LayoutVXCol const &layoutVXCol,
        AscendC::GlobalTensor<ElementY> const & gmY, LayoutY const &layoutY,
        Catlass::GemmCoord const &actualShape, Catlass::GemvCoord &actualShapeforX)
    {   
        ...
        auto layoutBInL1 = LayoutBInL1::template MakeLayout<ElementB>(L1TileShape::K, L1TileShape::N);
        auto layoutXInL1Col = LayoutXInL1Col::template MakeLayout<ElementX>(L1TileShapeforFT::K, L1TileShapeforFT::N);
        auto layoutVXInL1Col = LayoutXInL1Col::template MakeLayout<ElementX>(L1TileShapeforFT::K, L1XAlignHelper::M_ALIGNED);

        auto layoutInL0CTotal = LayoutCInL0::MakeLayoutInL0C(Catlass::MakeCoord(mRound, (nRound + nRoundforFT + L1XAlignHelper::M_ALIGNED)));

        auto layoutInL0CforFT = layoutInL0CTotal.GetTileLayout(Catlass::MatrixCoord(mRound, (nRoundforFT + L1XAlignHelper::M_ALIGNED)));
        ...
        auto layoutTileVXCol = layoutVXCol.GetTileLayout(Catlass::MakeCoord(kActual,uint32_t(1)));
        copyGmToL1VXCol(l1VXTensorList[l1VXListId], gmVX, layoutVXInL1Col, layoutTileVXCol);
        ...
        for(uint32_t kLoopIdx=0; kLoopIdx < kTileCount; kLoopIdx++)
        {
            ...
            if (kLoopIdx < kTileCount - 1){
                ...
                Catlass::MatrixCoord gmTileBOffset{kLoopIdxNext * L1TileShape::K, 0};
                Catlass::MatrixCoord gmTileXOffset{kLoopIdxNext * L1TileShapeforFT::K, 0};
                Catlass::MatrixCoord gmTileVXOffset{kLoopIdxNext * L1TileShapeforFT::K, 0};
                // uint32_t gmTilexOffsetNext{kLoopIdxNext * L1TileShape::K};
            ...
            for(int mPartIdx=0; mPartIdx < mPartLoop; mPartIdx++){
                ...
                for(int kPartIdx=0; kPartIdx < kPartLoop; kPartIdx++){
                    ...
                    for(int nPartIdx = 0; nPartIdx < nPartLoop; nPartIdx++){
                        ...
                        LayoutBInL0 layoutBInL0Total = 
                            LayoutBInL0::template MakeLayout<ElementB>(kPartActual, (nPartActual + nPartActualforX + L1XAlignHelper::M_ALIGNED));

                        LayoutBInL0 layoutBInL0 = layoutBInL0Total.GetTileLayout(Catlass::MakeCoord(kPartActual, nPartActual));
                            // MakeLayout<ElementB>();
                        
                        LayoutBInL0 layoutXInL0ColforABe = layoutBInL0Total.GetTileLayout(Catlass::MakeCoord(kPartActual, nPartActualforX));

                        LayoutBInL0 layoutXInL0ColforAe = layoutBInL0Total.GetTileLayout(Catlass::MakeCoord(kPartActual, L1XAlignHelper::M_ALIGNED));
                        ...
                        // Locate the current tile of matrix B on L1
                        Catlass::MatrixCoord l0BOffsetforB{0, 
                            nPartIdx * (nPartActual + nPartActualforX + L1XAlignHelper::M_ALIGNED)};

                        auto l0BTileforB = l0BTensor[layoutBInL0Total.GetOffset(l0BOffsetforB)];
                        
                        // Locate the current tile of matrix X on L1
                        Catlass::MatrixCoord l0BOffsetforXofABe{0, 
                            nPartIdx * (nPartActual + nPartActualforX + L1XAlignHelper::M_ALIGNED) + nPartActual};
                        
                        auto l0BTileforXofABe = l0BTensor[layoutBInL0Total.GetOffset(l0BOffsetforXofABe)]; 

                        Catlass::MatrixCoord l0BOffsetforXofAe{0, 
                            nPartIdx * (nPartActual + nPartActualforX + L1XAlignHelper::M_ALIGNED) + nPartActual + nPartActualforX};

                        auto l0BTileforXofAe = l0BTensor[layoutBInL0Total.GetOffset(l0BOffsetforXofAe)];
                        ...
                        Catlass::MatrixCoord l1VXOffsetforAe{kPartIdx * L0TileShape::K, 0};
                        auto l1VXTile = l1VXTensor[layoutVXInL1Col.GetOffset(l1VXOffsetforAe)];
                        ...
                        AscendC::WaitFlag<AscendC::HardEvent::M_MTE1>(l0BEventListforX[l0BListIdforBX]);
                        copyL1ToL0XCol(l0BTileforXofABe, l1XTile, layoutXInL0ColforABe, layoutXInL1Col);
                        // copyL1ToL0X(l0XTileforAe, l1VXTile, layoutXInL0forAe, layoutVXInL1);
                        copyL1ToL0XCol(l0BTileforXofAe, l1VXTile, layoutXInL0ColforAe, layoutVXInL1Col);
                        ...
                        // Locate the current tile on L0C
                        Catlass::MatrixCoord l0COffset{mPartIdx * L0TileShape::M, nPartIdx * (L0TileShape::N+L0TileShapeforFT::N+L1XAlignHelper::M_ALIGNED)};
                        // 获取当前局部输出
                        auto l0CTile = l0CTensor[layoutInL0CTotal.GetOffset(l0COffset)];
                        ...
                        // Perform calculation operations
                        tileMmad(l0CTile, l0ATileforA, l0BTensor, mPartActual, 
                            (nPartActual+nPartActualforX+L1XAlignHelper::M_ALIGNED), kPartActual, initC, unitFlag);
        ...
        // 将最终结果从 L0 的 CO1 输出到GM即可
        LayoutC layoutBlock = layoutC.GetTileLayout(actualShape.GetCoordMN());
        // L1XAlignHelper::M_ALIGNED
        LayoutY layoutBlockforFT = layoutY.GetTileLayout(Catlass::MakeCoord(actualShape.m(), nRoundforFT + 1));
        ...
        if constexpr (!ENABLE_UNIT_FLAG) {
            ...
            Catlass::MatrixCoord l0COffsetforFT{uint32_t(0), nRound};
            // 获取当前局部输出
            auto l0CTileforFT = l0CTensor[layoutInL0CTotal.GetOffset(l0COffsetforFT)];
            // 等待允许写入开始
            AscendC::WaitFlag<AscendC::HardEvent::M_FIX>(MMAD_EVENT_ID0);
            // 写入数据
            copyL0CToGm(gmC, l0CTileforC, layoutBlock, layoutInL0CforC);
            ...
            AscendC::WaitFlag<AscendC::HardEvent::M_FIX>(MMAD_ABE_EVENT_ID1);
            copyL0CToGmforABE(gmY, l0CTileforFT, layoutBlockforFT, layoutInL0CforFT);
            // 标记数据写入 cGM 已经完成
            AscendC::SetFlag<AscendC::HardEvent::FIX_M>(MMAD_EVENT_ID0);
            // 标记数据写入 cGM 已经完成
            AscendC::SetFlag<AscendC::HardEvent::FIX_M>(MMAD_ABE_EVENT_ID1);
            ...
            copyL0CToGm(gmC, l0CTileforC, layoutBlock, layoutInL0CforC, 0b11);
            copyL0CToGmforABE(gmY, l0CTileforFT, layoutBlockforFT, layoutInL0CforFT, 0b11);
        }
    }
...
protected:
    ...
    CopyGmToL1VXCol copyGmToL1VXCol;
