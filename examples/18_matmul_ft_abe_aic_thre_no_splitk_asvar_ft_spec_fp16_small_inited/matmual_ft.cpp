...
using GemvInTypeCforCE = op_bfloat16;
// op_bfloat16;
using GemvInTypeNforCE = bfloat16_t;
// bfloat16_t;
...
void Run(Options options) {
    ...
    size_t sizeBMean = lenBMean * sizeof(GemvOutTypeC);
    size_t sizeBMax = lenBMax * sizeof(GemvOutTypeC);

    size_t sizeAMean = lenARed * sizeof(GemvOutTypeC);
    size_t sizeAMax = lenARed * sizeof(GemvInTypeCforAB);
    ...
    std::vector<GemvInTypeCforAB> hostBMaxSlice(lenBE,(GemvInTypeCforAB)0.0f);
    ...
    std::vector<GemvInTypeCforAB> hostAMax(lenARed,(GemvInTypeCforAB)0.0f);


    std::vector<GemvOutTypeC> hostBMean(lenBMean, 0.0f);
    std::vector<GemvOutTypeC> hostBMax(lenBMax, 0.0f);
    ...
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&deviceBMaxSlice), sizeBE, ACL_MEM_MALLOC_HUGE_FIRST));
    ACL_CHECK(aclrtMemcpy(deviceBMaxSlice, sizeBE, hostBMaxSlice.data(), sizeBE, ACL_MEMCPY_HOST_TO_DEVICE));
    ...
    uint8_t* deviceBMean{nullptr};
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&deviceBMean), sizeBMean, ACL_MEM_MALLOC_HUGE_FIRST));

    uint8_t* deviceBMax{nullptr};
    ACL_CHECK(aclrtMalloc(reinterpret_cast<void**>(&deviceBMax), sizeBMax, ACL_MEM_MALLOC_HUGE_FIRST));
    ...
    // using L1TileShapeBE = GemvShape<256, 256>;
    // using L0TileShapeBE = GemvShape<64, 256>;
    ...
    using XType = Gemm::GemmType<GemvInTypeNforAB, LayoutX>;
    ...
    using TileCopyGemvAic = Gemv::Tile::TileCopyGemvAic<typename BeAICDispatchPolicy::ArchTag, BType, XType, YTypeBEAIC, BiasType>;
    using TileMmadGemvAic = Gemm::Tile::TileMmad<typename  BeAICDispatchPolicy::ArchTag, XType, BType, BiasType>;
    ...
    */
    ...
    using BlockFTGemvAIC = Gemv::Block::BlockFTGemvBe<BeAICDispatchPolicy, 
        ...
        BType, XType, YTypeBEAIC, BiasType, TileCopyGemvAic, TileMmadGemvAic>;

    using ZType = Gemm::GemmType<GemvOutTypeN, LayoutZ>;

    constexpr uint32_t computeLength = 8192;

    using YType = Gemm::GemmType<GemvInTypeNforAB, LayoutY>;
    ...
    */
    ...
    using TileFaultSum = Gemv::Tile::TileFaultSum<ArchTag, FT_REDUCE_TYPE::SUM_MAX_MIXED, AType, ZType>;
    ...
    */
    ...
    // using SliceSumTileCopy = Gemv::Tile::TileCopyMatrixSliceSumAiv<typename SliceSumDispatchPolicy::ArchTag,MYType,ZType,BiasType>;

    // static const bool MakeDich = true;
    // using TileSliceSum = Gemv::Tile::TileSliceSum<typename SliceSumDispatchPolicy::ArchTag,MYType,ZType,BiasType,MakeDich>;

    // using BlockSliceSum = Gemv::Block::BlockSliceSum<SliceSumDispatchPolicy,SliceSumUBTileShape,MYType,ZType,BiasType,SliceSumTileCopy,TileSliceSum>;
    ...
    */
    ...
    using TileFaultSumBReduce = Gemv::Tile::TileFaultSum<ArchTag, FT_REDUCE_TYPE::SUM_MAX, BType, ZType>;
    using TileFaultCopyBReduce = Gemv::Tile::TileCopyGemvAiv<ArchTag, BType, YType, ZType>;
    ...
    using UBTileShapeABE = GemvShape<64,256>;
    using UBBlockShapeABE = GemvShape<L1TileShape::M,UBTileShapeABE::N>;
    ...
    */
    ...
    using TileFaultCopyABE = Gemv::Tile::TileCopyGemvThreCompFusedAiv<ArchTag, 
        AType, CType, YType, ZType, COMPZType, void>;
    using TileFaultVmadFT = Gemv::Tile::TileVmad<ArchTag, AType, YType, ZType>;

    using ThreCalcDispatchPolicy = Gemm::GemvAtlasA2;
    using ThreCalcUBTileShape = GemvShape<64,L0TileShape::N>;
    using ThreCalcUBTileShapeTotal = GemvShape<L1TileShape::M,L1TileShape::N>;

    // using ThreCalcTileCopy = Gemv::Tile::TileCopyMatrixReduceAivFused<typename ThreCalcDispatchPolicy::ArchTag, CType, YType, ZType, COMPZType, BiasType>;
    ...
    */
    ...
    using TileStdEst = Gemv::Tile::TileStdEst<
        typename ThreCalcDispatchPolicy::ArchTag,
        ZType,
        ZType
    >;

    using BlockFTGemvAIV = Gemv::Block::BlockFTGemvNoSplitK<
        GemvDispatchPolicy,
        Gemv::helper::FT_THRESHOLD_ALGORITHM::ASVAR,
        Gemv::helper::FT_AIV_PIPE_FUSE_TYPE::THRE_FUSED, 
        Gemv::helper::FT_ENC_TYPE::RCE, Gemv::helper::FT_COMP_TYPE::RSUB,
        UBTileShapeABE, UBBlockShapeABE, L1TileShape,
        AType, YType, ZType, COMPZType, void, TileFaultCopyABE, TileFaultVmadFT,
        TileThreCalc, ThreCalcTileVmuls, TileStdEst>;

    using UBTileShape = GemmShape<L1TileShapeAB::M, L1TileShapeAB::N, L1TileShapeAB::K>;
    using TileFaultCopy = Gemv::Tile::TileCopyGemvAiv<ArchTag, AType, XType, ZType>;
    using TileFaultVmad = Gemv::Tile::TileVmad<ArchTag, AType, XType, ZType>;
   
    using BlockSumGemv = Gemv::Block::BlockSumGemv<GemvDispatchPolicy, UBTileShape, AType, XType, ZType, void, TileFaultCopy, TileFaultVmad, TileVmuls>;
    ...
    using UBTileShapeCOMPSUB = GemvShape<1,1024>;
    ...
    using CompareBlockSUB = Gemv::Block::BlockCompare<COMPDispatchPolicy,FT_COMP_TYPE::SUB, UBTileShapeCOMPSUB, COMPZType, ZType, ZType, TileCompareCopySUB>;
    ...
    */
    // , BlockThresholdCalc
    // CompareBlockSUB
    using MatmulFTKernel = CubeSelf::Gemm::Kernel::MatmulAsVarABonAicNoSplitMixedSpec<
    ...
    */
    ...
    struct Arguments {
        ...
        size_t elementSize; GM_ADDR ptrX; GM_ADDR ptrXV;
        GM_ADDR ptrA; GM_ADDR ptrB; GM_ADDR ptrC;
        GM_ADDR ptrZRow; GM_ADDR ptrZCol;
        GM_ADDR ptrZRow2; GM_ADDR ptrZCol2;
        GM_ADDR ptrCOMPZRow; GM_ADDR ptrCOMPZCol;
        GM_ADDR ptrBE; GM_ADDR ptrBMaxSlice;
        GM_ADDR ptrBMean; GM_ADDR ptrBMax;
        GM_ADDR ptrAMean; GM_ADDR ptrAMax;
        GM_ADDR ptrThreZ; FT_ENC_TYPE enc_type;
        uint32_t UbNum; bool OutputWorkspace;
        ...
        bool outputThre; bool outputABE; uint32_t SplitKNum;
    };
    */

    typename MatmulFTKernel::Arguments arguments{
        options.problemGemmShape, 
        options.problemShape, sizeof(GemvInTypeCforAB), 
        deviceX, deviceXV, deviceA, deviceB, deviceC, deviceZRow, 
        deviceZCol, deviceDRow, deviceDCol, deviceCOMPRow, 
        deviceCOMPCol, deviceBE, deviceBMaxSlice, 
        deviceBMean, deviceBMax,
        deviceAMean, deviceAMax,
    ...
    ACL_CHECK(aclrtMemcpy(hostBMean.data(), sizeBMean, deviceBMean, sizeBMean, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(hostBMax.data(), sizeBMax, deviceBMax, sizeBMax, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(hostBE.data(), sizeBE, deviceBE, sizeBE, ACL_MEMCPY_DEVICE_TO_HOST));
    ACL_CHECK(aclrtMemcpy(hostBMaxSlice.data(), sizeBE, deviceBMaxSlice, sizeBE, ACL_MEMCPY_DEVICE_TO_HOST));
    ...
    std::vector<GemvOutTypeC> hostBMeanGolden(lenBMean,(GemvOutTypeC)0.0f);
    std::vector<GemvOutTypeC> hostBMaxGolden(lenBMax, (GemvOutTypeC)0.0f);
    std::vector<GemvInTypeCforAB> hostYForAB(static_cast<uint32_t>(k * splitNnum), (GemvInTypeCforAB)0.0f);
    ...
    golden::ComputeGemvSlice(options.problemShape, BESliceShape, alpha, beta,
         hostC, layoutC, hostXV, layoutXRow, hostGoldenCRow, layoutZHost, hostGoldenCRow);
    ...
    */
    ...
    std::vector<uint64_t> errorIndices = golden::CompareData(hostBE, hostYForAB, problemShapeBE.m());
    ...
    //     printf("Correct: Computed: %f, Actual: %f\n",(float)hostBE[ii],(float)hostYForAB[ii]);
    // }
    ...
    errorIndices = golden::CompareData(hostDRow, hostGoldenRow, problemShapeABE.n());
    ...
    errorIndices = golden::CompareData(hostBMean, hostBMeanGolden, splitNnum);

    for(int j=0; j < splitNnum; j++){
        printf("Expect B Mean[%d]: %f\n",j,hostBMeanGolden[j]);
        printf("Actual B Mean[%d]: %f\n",j,hostBMean[j]);
    }

    if (errorIndices.empty()) {
        std::cout << "B Reduce Mean Compare success." << std::endl;
    } else {
        std::cerr << "B Reduce Mean Compare failed." << std::endl;
    }
    
    errorIndices = golden::CompareData(hostBMax, hostBMaxGolden, splitNnum);

    if (errorIndices.empty()) {
        std::cout << "B Reduce Max Compare success." << std::endl;
    } else {
        std::cerr << "B Reduce Max Compare failed." << std::endl;
    }

    for(int j=0; j < splitNnum; j++){
        printf("Expect B Max[%d]: %f\n",j,hostBMaxGolden[j]);
        printf("Actual B Max[%d]: %f\n",j,hostBMax[j]);
    }
    ...
    */
    ...
    // for(int j=0; j < splitNnum; j++){
    //     printf("Expect Mean[%d]: %f\n",j,hostBMeanGolden[j]);
    //     printf("Actual Mean[%d]: %f\n",j,hostBMean[j]);
    // }
    ...
    // for(int j=0; j < splitNnum; j++){
    //     printf("Expect Max[%d]: %f\n",j,hostBMaxGolden[j]);
    //     printf("Actual Max[%d]: %f\n",j,hostBMax[j]);
    // }
    ...
    // const std::vector<ElementA> &dataA, const LayoutA &layoutA,
    // std::vector<ElementA> &dataAMean,
    // std::vector<ElementA> &dataAMax,
    ...
    // errorIndices = golden::GetErrorDataAndIndexSliceWithThreshold(
    //     options.problemShape,
    //     hostCOMPRow, hostGoldenCOMPRow,hostZRow, hostGoldenRow, hostThreGolden, 
    //     lenZRow, "CE", "ABE", totalErrorIdxRow, totalErrorIdxRow_m, totalErrorIdxRow_n,
    //     totalErrorDataRow, totalFailThresholds);
    ...
    // // printf("Total Data Id:");
    // // for(int32_t kk=0; kk < static_cast<int32_t>(totalErrorIdxRow.size()); kk++){
    // //     printf(" %lu",totalErrorIdxRow[kk]);
    // // }
    // // printf("\n");
    ...
    // // std::vector<uint8_t> hostGoldenCOMPCol(lenZCol,255);
    ...
    // // printf("%f\n", hostZCol[0]);
    // // printf("%f\n", hostDCol[0]);
    // // printf("Column Verify with Method: SUB (threshold: %f)\n", threshold);
    // // errorIndices = golden::CompareDataWithIndex(hostCOMPCol, hostGoldenCOMPCol,
    // //     hostZCol, hostDCol, lenZCol, "ETC", "ETAB");
    // // if (errorIndices.empty()) {
    ...
    ACL_CHECK(aclrtFree(deviceBMax));
    ACL_CHECK(aclrtFree(deviceBMean));
