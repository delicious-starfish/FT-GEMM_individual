...
#ifndef CATLASS_GEMV_BLOCK_BLOCK_GEMV_ASVAR_AIV_NO_SPLIT_HPP_RELIEVE
#define CATLASS_GEMV_BLOCK_BLOCK_GEMV_ASVAR_AIV_NO_SPLIT_HPP_RELIEVE
...
public:
    ...
    using TileFaultMeanMax = Gemv::Tile::TileFaultSum<ArchTag, FT_REDUCE_TYPE::SUM_MAX_ABE, AType_, YType_>;
    ...
    */
    ...
    static constexpr uint32_t Maxbuf_SIZE_ = 4 * 1024;
    static constexpr uint32_t Zbuf_SIZE_ = 2 *1024;

    static constexpr uint32_t ELE_NUM_PER_BLK_FOR_B = BYTE_PER_BLK / sizeof(ElementY);
    ...
    BlockFTGemvNoSplitK(Arch::Resource<ArchTag> &resource, uint32_t UBufAddrStart = 0)
    {
        ...
        // + Stdbuf_SIZE_ 
        // + Stdbuf_SIZE_ 
        uint32_t UbMeanOffset = UBufAddrStart + Abuf_SIZE_ + Xbuf_SIZE_ + Ybuf_SIZE_ + InCbuf_SIZE_ + Threbuf_SIZE_;
        uint32_t UbMaxOffset = UBufAddrStart + Abuf_SIZE_ + Xbuf_SIZE_ + Ybuf_SIZE_ + InCbuf_SIZE_ + Threbuf_SIZE_ + Meanbuf_SIZE_;
        uint32_t UbZOffset = UBufAddrStart + Abuf_SIZE_ + Xbuf_SIZE_ + Ybuf_SIZE_ + InCbuf_SIZE_ + Threbuf_SIZE_ + Meanbuf_SIZE_ + Maxbuf_SIZE_;
        

        uint32_t UbWOffset = UBufAddrStart + Abuf_SIZE_ + Xbuf_SIZE_ + Ybuf_SIZE_ + InCbuf_SIZE_ + Threbuf_SIZE_ + Meanbuf_SIZE_ + Maxbuf_SIZE_ + Zbuf_SIZE_;
        uint32_t UbWMaxOffset = UBufAddrStart + Abuf_SIZE_ + Xbuf_SIZE_ + Ybuf_SIZE_ + InCbuf_SIZE_ + Threbuf_SIZE_ + Meanbuf_SIZE_ + Maxbuf_SIZE_ + Zbuf_SIZE_ + sum_workspace_SIZE_;
        ...
        for (uint32_t i = 0; i < STAGES; i++) {
            ...
            UbWMeanMaxTensorList[i] = UbWTensorList[i].template ReinterpretCast<ElementA>();
            ...
            UbMeanTensorList[i] = resource.ubBuf.template GetBufferByByte<ElementY>(UbMeanOffset + i * (Meanbuf_SIZE_ / 2));
            UbMaxTensorList[i] = resource.ubBuf.template GetBufferByByte<ElementY>(UbMaxOffset + i * (Maxbuf_SIZE_ / 2));
    ...
    void operator()(
        ...
        uint32_t aiv_part_num, Catlass::Arch::CrossCoreFlagWithReverse<> & flagAicFinishStore)
    {
        TileMRound = RoundUp(UBTileShape::M, UBAlignHelper::ALIGN);
        TileNRound = RoundUp(UBTileShape::N, UBAlignHelper::ALIGN);
        ...
        for(uint32_t mLoopIdx = 0; mLoopIdx < Mloop; mLoopIdx++){
            ...
            AscendC::Duplicate<ElementY>(UbMeanTensor, (ElementY)0.0, m_actual);
            AscendC::Duplicate<ElementY>(UbMaxTensor, (ElementY)0.0, m_actual);
            ...
            tileVmulsforMean(UbMeanTensor, UbMeanTensor, (ElementY)A_row_scale_ratio, m_actual);
            AscendC::PipeBarrier<PIPE_V>();
            ...
            */
            ...
            tileStdEst(UbMaxTensor, UbMeanTensor, UbMaxTensor, layoutStdInUb, layoutTileStd,(ElementY)std_est_A_row_ratio);
...
protected:
    ...
    AscendC::LocalTensor<ElementA> UbWMeanMaxTensorList[STAGES]; 
    ...
    AscendC::LocalTensor<ElementY> UbMeanTensorList[STAGES]; // 存储 A 的行mean 向量
    AscendC::LocalTensor<ElementY> UbMaxTensorList[STAGES]; //  存储 A 的行max 向量
    ...
    ElementY B_slice_mean, B_slice_max, B_slice_std, B_slice_mean_abs;
