#ifndef CATLASS_GEMV_BLOCK_BLOCK_AIC_FT_HPP
#define CATLASS_GEMV_BLOCK_BLOCK_AIC_FT_HPP
...
public:
    ...
    using CopyGmToL1BCol = typename TileCopy_::CopyGmToL1BCol;

    using CopyL1ToL0A = typename TileCopy_::CopyL1ToL0A;
    using CopyL1ToL0B = typename TileCopy_::CopyL1ToL0B;
    using CopyL1ToL0BCol = typename TileCopy_::CopyL1ToL0BCol;
    ...
    static constexpr uint32_t L1A_SIZE =
        ...
        (ENC_TYPE_ == FT_ENC_TYPE::ETC)   ? (16 * L1TileShape::M * sizeof(ElementX)) :
    ...
    static constexpr uint32_t L0A_TILE_SIZE =
        ...
        (ENC_TYPE_ == FT_ENC_TYPE::ETC)   ? (L1XAlignHelper::M_ALIGNED * L0TileShape::M * sizeof(ElementX)) :
    ...
    /// Construct
    CATLASS_DEVICE
    BlockFTGemv(Arch::Resource<ArchTag>& resource, uint32_t l1BufAddrStart = 0)
    {
    ...
    void FillScalarInLocal(uint32_t coreID, ElementX fill_value,FT_ENC_TYPE enc_type)
    {
        ...
        if(enc_type == FT_ENC_TYPE::CE){
            ...
            calCount = L1XAlignHelper::M_ALIGNED * L0TileShape::M;
        ...
        LayoutX const& layoutX,FT_ENC_TYPE enc_type)
    {
        ...
        if(enc_type == FT_ENC_TYPE::CE) {
            ...
            L1TileMSize = L1XAlignHelper::M_ALIGNED;
            L1TileNSize = L1TileShape::M;
            
            L0TileMSize = L1XAlignHelper::M_ALIGNED;
            L0TileNsize = L0TileShape::M;
        } else if(enc_type == FT_ENC_TYPE::BOTHC) {
            L1TileMSize = L1XAlignHelper::M_ALIGNED;
    ...
    //     auto layoutInL0C = LayoutYInL0::MakeLayoutInL0C(MatrixCoord(L1XAlignHelper::M_ALIGNED, actualShape.n()));

    //     uint32_t mTileCount = CeilDiv<L1TileShape::M>(actualShape.m());
    //     uint32_t mTileCountNext = CeilDiv<L1TileShape::M>(actualShapeNext.m());
    ...
    //     // 每次处理一列的数据
    //     uint32_t firstTileIdx = startTileIdx % mTileCount;
    //     uint32_t lastTileIdx = (startTileIdx + mTileCount - 1) % mTileCount;
    //     uint32_t firstTileIdxNext = startTileIdx % mTileCountNext;

    //     uint32_t mActual =
    //         (firstTileIdx < mTileCount - 1) ? L1TileShape::M : (actualShape.m() - firstTileIdx * L1TileShape::M);
    //     uint32_t mRound = RoundUp<L1AAlignHelper::N_ALIGNED>(mActual);

    //     // main loop
    //     for (uint32_t mLoopIdx = 0; mLoopIdx < mTileCount; mLoopIdx++) {
    //         uint32_t shuffleKIdx = (startTileIdx + mLoopIdx) % mTileCount;
    //         if (shuffleKIdx == firstTileIdx && isFirstBlock) {
    //             MatrixCoord gmTileAOffset{shuffleKIdx * L1TileShape::M, 0};

    //             auto gmTileA = gmBlockA[layoutA.GetOffset(gmTileAOffset)];

    //             // load first matrix A tile from GM to L1
    //             AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(l1BEventList[l1ListId]);
    //             auto layoutTileA = layoutA.GetTileLayout(MakeCoord(mRound, actualShape.n()));
    //             copyGmToL1BCol(l1BTensorList[l1ListId], gmTileA, layoutAInL1, layoutTileA);
    //             AscendC::SetFlag<AscendC::HardEvent::MTE2_MTE1>(l1BEventList[l1ListId]);
    //         }

    //         uint32_t l1ListIdNext = (l1ListId + 1) % STAGES;
    //         uint32_t mActualNext{0};
    //         uint32_t mRoundNext{0};

    //         // preload next tile from GM to L1
    //         if (shuffleKIdx != lastTileIdx) {
    //             uint32_t shuffleKIdxNext = (startTileIdx + mLoopIdx + 1) % mTileCount;
    //             mActualNext = (shuffleKIdxNext < mTileCount - 1) ? L1TileShape::M
    //                                                              : (actualShape.m() - shuffleKIdxNext * L1TileShape::M);
    //             mRoundNext = RoundUp<L1AAlignHelper::N_ALIGNED>(mActualNext);

    //             // Get L1 tensor
    //             auto l1BTensor = l1BTensorList[l1ListIdNext];

    //             // Get GM tile
    //             MatrixCoord gmTileAOffset{shuffleKIdxNext * L1TileShape::M, 0};

    //             auto gmTileA = gmBlockA[layoutA.GetOffset(gmTileAOffset)];

    //             // load Matrix A tile from GM to L1
    //             AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(l1BEventList[l1ListIdNext]);
    //             auto layoutTileA = layoutA.GetTileLayout(MakeCoord(mRoundNext, actualShape.n()));
    ...
    //             // Get GM tensor for next stage
    //             mActualNext = (firstTileIdxNext < mTileCountNext - 1)
    //                 ? L1TileShape::M : (actualShapeNext.m() - firstTileIdxNext * L1TileShape::M);
    //             mRoundNext = RoundUp<L1AAlignHelper::N_ALIGNED>(mActualNext);

    //             // Get GM tile
    //             MatrixCoord gmTileAOffset{firstTileIdxNext * L1TileShape::M, 0};

    //             auto gmTileA = gmNextBlockA[layoutA.GetOffset(gmTileAOffset)];

    //             // load Matrix A tile from GM to L1
    //             AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(l1BEventList[l1ListIdNext]);
    //             auto layoutTileA = layoutA.GetTileLayout(MakeCoord(mRoundNext, actualShapeNext.n()));
    ...
    //         uint32_t mRound = RoundUp<L1AAlignHelper::N_ALIGNED>(mActual);
    //         uint32_t mPartLoop = CeilDiv<L0TileShape::M>(mActual);

    //         for (uint32_t mPartIdx = 0; mPartIdx < mPartLoop; mPartIdx++) {
    //             uint32_t mPartActual =
    //                 (mPartIdx < mPartLoop - 1) ? L0TileShape::M : (mActual - mPartIdx * L0TileShape::M);
    ...
    //             // Locate the current tile on L0B
    //             auto l0BTile = l0BTensorList[l0BListId];
    //             LayoutAInL0Col layoutAInL0 = LayoutAInL0Col::template MakeLayout<ElementA>(mPartActual, L0TileShape::N);

    //             MatrixCoord l1AOffset{mPartIdx * L0TileShape::M, 0};
    //             auto l1BTile = l1BTensor[layoutAInL1.GetOffset(l1AOffset)];
    ...
    //             // If the current tile is the first tile on the k axis, the accumulator needs to be reset to 0
    //             bool initC = ((mLoopIdx == 0) && (mPartIdx == 0));

    //             AscendC::WaitFlag<AscendC::HardEvent::MTE1_M>(l0BEventList[l0BListId]);
    //             tileMmad(l0CTile, l0ATile, l0BTile, L1XAlignHelper::M_ALIGNED, L0TileShape::N, mPartActual, initC);
    //             AscendC::SetFlag<AscendC::HardEvent::M_MTE1>(l0BEventList[l0BListId]);
    ...
    //     // copy block out
    //     LayoutY layoutBlock = layoutY.GetTileLayout(MakeCoord(uint32_t(1), actualShape.n()));
