#ifndef CATLASS_GEMV_BLOCK_BLOCK_SUM_GEMV_AIV_NOW_HPP
#define CATLASS_GEMV_BLOCK_BLOCK_SUM_GEMV_AIV_NOW_HPP
...
public:
    ...
    using TileVmadCol = Gemv::Tile::TileVmad<typename DispatchPolicy::ArchTag, AColType, XType_, YType_, BiasType_>;
    ...
    /// Construct
    CATLASS_DEVICE
    BlockSumGemv(Arch::ResourceAIV<ArchTag> &resource, uint32_t UBufAddrStart = 0)
    {
        uint32_t UbAOffset = UBufAddrStart;
        uint32_t UbXOffset = UBufAddrStart + Abuf_SIZE_;
        uint32_t UbYOffset = UBufAddrStart + Abuf_SIZE_ + Xbuf_SIZE_;
        ...
        float beta)
    {
        ...
        auto layoutAInUb = layoutA.GetTileLayout(MakeCoord(TileMRound, TileNRound));
        auto layoutTileA = layoutA.GetTileLayout(MakeCoord(m_actual, n_actual));
        //LayoutAInUbCol layoutAInUb;
        //layoutAInUb = layoutAInUb.GetTileLayout(MakeCoord(TileNRound, TileMRound));
        //auto layoutAInUb = Catlass::layout::ColumnMajor(TileMRound, TileNRound, RoundUp<BYTE_PER_C0 / sizeof(ElementA)>(TileNRound));

        matrixCopyGmToUbCol(UbATensorList[UbInListId], gmA, layoutAInUb, layoutTileA);
        AscendC::SetFlag<AscendC::HardEvent::MTE2_V>((event_t)(UbInAEventList[UbInListId]));
        // main loop
        uint32_t Nloop = CeilDiv(actualShape.n(), TileNRound);
        for (uint32_t LoopIdx = 0; LoopIdx < Nloop; LoopIdx++) {
            ...
            if (LoopIdx < Nloop - 1) {
                ...
                matrixCopyGmToUbCol(matrixTensor, gmA[LoopIdxNext * strideA], layoutAInUb, layoutTileA);
                AscendC::SetFlag<AscendC::HardEvent::MTE2_V>((event_t)(UbInAEventList[UbInListIdNext]));
            }
            AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>((event_t)(UbInXEventList[UbInListId]));
            tileVmuls(UbXTensorList[UbInListId], UbXTensorList[UbInListId], (ElementA)alpha, x_actual);
            ...
            auto layoutComputeInUb = layoutA.GetTileLayout(MakeCoord(TileMRound, TileNRound));
            auto layoutTileCompute = layoutA.GetTileLayout(MakeCoord(m_actual, n_actual));
            tileVmadCol(UbYTensorList[UbOutListId],
                UbXTensorList[UbInListId],
                UbATensorList[UbInListId],
                UbWTensorList[UbInListId],
                layoutComputeInUb,
