...
#ifndef CATLASS_GEMV_BLOCK_BLOCK_GEMV_ASVAR_CE_AIV_NO_SPLIT_HPP_SPEC_SIMPLIFIED_WAIT
#define CATLASS_GEMV_BLOCK_BLOCK_GEMV_ASVAR_CE_AIV_NO_SPLIT_HPP_SPEC_SIMPLIFIED_WAIT
...
struct BlockFTGemvCENoSplitK <
    ...
    Gemv::helper::FT_ABE_TYPE::WAIT_CENTRAL_BLOCK,
    UBTileShape_,
    UBBlockShape_,
    L1TileShape_,
    AType_,
...
public:
    ...
    static constexpr FT_ABE_TYPE ABE_TYPE = FT_ABE_TYPE::WAIT_CENTRAL_BLOCK;
    ...
    BlockFTGemvCENoSplitK(Arch::Resource<ArchTag> &resource, uint32_t UBufAddrStart = 0)
    {
        ...
        for (uint32_t i = 0; i < STAGES; i++) {
            ...
            AscendC::SetFlag<AscendC::HardEvent::V_MTE2>(UbInAMaxEventList[i]);
    ...
    ~BlockFTGemvCENoSplitK()
    {
        for (uint32_t i = 0; i < STAGES; i++) {
            ...
            AscendC::WaitFlag<AscendC::HardEvent::V_MTE2>(UbInAMaxEventList[i]);
    ...
    // AscendC::GlobalTensor<ElementY> const &gmStdZ, bool outputStd,
    CATLASS_DEVICE
    void operator()(
        ...
        Catlass::Arch::CrossCoreFlagWithReverse<> & flagAicFinishStore)
    {
        ...
        B_slice_var = B_slice_var * (ElementY)std_est_B_ratio;
        ...
        AscendC::WaitFlag<AscendC::HardEvent::V_MTE2>((event_t)(UbInAMaxEventList[UbOutListId]));
        ...
        for(uint32_t mLoopIdx = 0; mLoopIdx < Mloop; mLoopIdx++){
            ...
            if (mLoopIdx < (Mloop -1)) {
                ...
                AscendC::WaitFlag<AscendC::HardEvent::V_MTE2>((event_t)(UbInAMaxEventList[UbOutListIdNext]));
            ...
            */
            ...
            tileStdEst(UbAMaxTensor, 
                UbAMaxTensor, 
                layoutStdInUb, 
                layoutTileStd, std_est_A_row_ratio);
            /*
            void operator()(
                AscendC::LocalTensor<ElementY> dstTensor,
            ...
            */
            ...
            // AscendC::PipeBarrier<PIPE_V>();
            tileThreCalc(
                UbThreTensor, UbAMaxTensor, UbWTensorList[UbOutListId], 
                layoutThreInUb, layoutTileThre, 
            ...
            auto UbABeTensor = UbABeTensorList[UbOutListId];
            vecCopyGmToUbInY(UbABeTensor, gmABe[mLoopOffset], m_actual);
            ...
            */
            ...
            //     auto layoutComputeStdInUb = layoutThre.GetTileLayout(TensorCoord(m_actual));
            //     vecCopyUbToGmforThre(gmStdZ[mLoopOffset], 
            //         UbAMaxTensorforYList[UbOutListId],layoutDstYStd, 
            //         layoutComputeStdInUb);
            // }
            ...
            AscendC::SetFlag<AscendC::HardEvent::V_MTE2>((event_t)(UbInAMaxEventList[UbOutListId]));
