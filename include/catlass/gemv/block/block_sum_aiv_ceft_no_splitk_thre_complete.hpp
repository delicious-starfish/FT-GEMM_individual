...
#ifndef CATLASS_GEMV_BLOCK_BLOCK_GEMV_ASVAR_CE_AIV_NO_SPLIT_HPP_SPEC
#define CATLASS_GEMV_BLOCK_BLOCK_GEMV_ASVAR_CE_AIV_NO_SPLIT_HPP_SPEC
...
public:
    ...
    static constexpr uint32_t Maxbuf_SIZE_forX_ = 2 * 1024;
    ...
    BlockFTGemvCENoSplitK(Arch::Resource<ArchTag> &resource, uint32_t UBufAddrStart = 0)
    {
        ...
        uint32_t UbAMaxOffset_forX =  UBufAddrStart + Cbuf_SIZE_ + Ybuf_SIZE_ + ABebuf_SIZE_ + Meanbuf_SIZE_;
        uint32_t UbAMaxOffset_forY =  UBufAddrStart + Cbuf_SIZE_ + Ybuf_SIZE_ + ABebuf_SIZE_ + Meanbuf_SIZE_ + Maxbuf_SIZE_forX_;

        uint32_t UbThreOffset = UBufAddrStart + Cbuf_SIZE_ + Ybuf_SIZE_ + ABebuf_SIZE_ + Meanbuf_SIZE_ + Maxbuf_SIZE_forX_ + Maxbuf_SIZE_forY_;
        
        uint32_t UbZOffset = UBufAddrStart + Cbuf_SIZE_ + Ybuf_SIZE_ + ABebuf_SIZE_ + Meanbuf_SIZE_ + Maxbuf_SIZE_forX_ + Maxbuf_SIZE_forY_ + Threbuf_SIZE_;

        uint32_t UbWOffset = UBufAddrStart + Cbuf_SIZE_ + Ybuf_SIZE_ + ABebuf_SIZE_ + Meanbuf_SIZE_ + Maxbuf_SIZE_forX_ + Maxbuf_SIZE_forY_ + Threbuf_SIZE_ + Zbuf_SIZE_;
        uint32_t UbWOffset_thre = UBufAddrStart + Cbuf_SIZE_ + Ybuf_SIZE_ + ABebuf_SIZE_ + Meanbuf_SIZE_ + Maxbuf_SIZE_forX_ + Maxbuf_SIZE_forY_ + Threbuf_SIZE_ + Zbuf_SIZE_ + workspace_SIZE_;
        ...
        for (uint32_t i = 0; i < STAGES; i++) {
            ...
            UbAMaxTensorforXList[i] = resource.ubBuf.template GetBufferByByte<ElementX>(UbAMaxOffset_forX + i * (Maxbuf_SIZE_forX_ / 2));
    ...
    void operator()(
        ...
        AscendC::GlobalTensor<ElementY> const &gmAMean, LayoutX const &layoutAforFT,
        AscendC::GlobalTensor<ElementY> const &gmY, 
        AscendC::GlobalTensor<ElementY> const &gmABe, LayoutY const &layoutY,
        AscendC::GlobalTensor<ElementY> const &gmBMean,
        AscendC::GlobalTensor<ElementY> const &gmBMax, LayoutX const &layoutBforFT,
        ...
        Catlass::Arch::CrossCoreFlagWithReverse<> & flagAicFinishStore)
    {
        ...
        B_slice_mean_abs = (B_slice_mean < (ElementY)0.0) ? ((ElementY)0.0 - B_slice_mean) : B_slice_mean; 
        ...
        auto UbAMaxTensorforX = UbAMaxTensorforXList[UbOutListId];
        vecCopyGmToUb(UbAMaxTensorforX, gmAMax[M_start_offset], m_actual);
        ...
        AscendC::SetFlag<AscendC::HardEvent::MTE2_V>((event_t)(UbInAMaxEventList[UbOutListId]));
        AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>((event_t)(UbInAMaxEventList[UbOutListId]));
        ...
        for(uint32_t mLoopIdx = 0; mLoopIdx < Mloop; mLoopIdx++){
            ...
            if (mLoopIdx < (Mloop -1)) {
                ...
                // if(NEED_CAST_FOR_RED){
                //     auto UbAMaxTensorforXNext = UbAMaxTensorforXList[UbOutListIdNext];
                //     vecCopyGmToUb(UbAMaxTensorforXNext, gmAMax[mLoopOffsetNext], m_actual_next);
                // }else{
                //     auto UbAMaxTensorforXNext = UbAMaxTensorforYList[UbOutListIdNext];
                //     vecCopyGmToUbInY(UbAMaxTensorforXNext, gmAMax[mLoopOffsetNext], m_actual_next);
                // }
                auto UbAMaxTensorforXNext = UbAMaxTensorforXList[UbOutListIdNext];
                vecCopyGmToUb(UbAMaxTensorforXNext, gmAMax[mLoopOffsetNext], m_actual_next);
                ...
                AscendC::SetFlag<AscendC::HardEvent::MTE2_V>((event_t)(UbInAMaxEventList[UbOutListIdNext]));
                AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>((event_t)(UbInAMaxEventList[UbOutListIdNext]));
            ...
            tileStdEst(UbAMaxTensor, UbAMeanTensor, UbAMaxTensor, layoutStdInUb, layoutTileStd,(ElementY)std_est_A_row_ratio);
...
protected:
    ...
    AscendC::LocalTensor<ElementX> UbAMaxTensorforXList[STAGES]; //  存储 A 的行 max 向量（ElementX）
    ...
    ElementY B_slice_mean;
    ElementY B_slice_max;
    ElementY B_slice_std;
    ElementY B_slice_mean_abs;
