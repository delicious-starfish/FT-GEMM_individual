...
#ifndef CATLASS_GEMV_BLOCK_BLOCK_GEMV_ASVAR_CE_AIV_NO_SPLIT_HPP_SPEC_ROBUST_PRELOAD_FI
#define CATLASS_GEMV_BLOCK_BLOCK_GEMV_ASVAR_CE_AIV_NO_SPLIT_HPP_SPEC_ROBUST_PRELOAD_FI
...
public:
    ...
    using VecCopyUbToGmforFIIndex = typename TileCopy_::VecCopyUbToGmforFIIndex;
    using VecCopyUbToGmforFIData = typename TileCopy_::VecCopyUbToGmforFIData;
    ...
    static constexpr uint32_t FI_ID_buf_SIZE_ = 2 * 1024;
    static constexpr uint32_t FI_Data_buf_SIZE_ = 2 * 1024;
    ...
    CATLASS_DEVICE
    BlockFTGemvCENoSplitKPreloadFI() {}

    /// Construct
    CATLASS_DEVICE
    BlockFTGemvCENoSplitKPreloadFI(Arch::Resource<ArchTag> &resource, uint32_t UBufAddrStart = 0)
    {
        ...
        uint32_t UbFIOffset_ID = UBufAddrStart + Cbuf_SIZE_ + Ybuf_SIZE_ + ABebuf_SIZE_ + Meanbuf_SIZE_ + Minbuf_SIZE_forY_ + Maxbuf_SIZE_forY_ + Threbuf_SIZE_ + Zbuf_SIZE_ + workspace_SIZE_ + thre_workspace_SIZE_;
        uint32_t UbFIOffset_Data = UBufAddrStart + Cbuf_SIZE_ + Ybuf_SIZE_ + ABebuf_SIZE_ + Meanbuf_SIZE_ + Minbuf_SIZE_forY_ + Maxbuf_SIZE_forY_ + Threbuf_SIZE_ + Zbuf_SIZE_ + workspace_SIZE_ + thre_workspace_SIZE_ + FI_ID_buf_SIZE_;
        ...
        for (uint32_t i = 0; i < STAGES; i++) {
            ...
            UbFIIndexTensorList[i] = resource.ubBuf.template GetBufferByByte<int32_t>(UbFIOffset_ID + i * (FI_ID_buf_SIZE_ / 2));
            UbFIFlipDataTensorList[i] = resource.ubBuf.template GetBufferByByte<ElementA>(UbFIOffset_Data + i * (FI_Data_buf_SIZE_ / 2));
            ...
            AscendC::SetFlag<AscendC::HardEvent::S_MTE2>(UbFICEventList[i]);
        }
    ...
    /// Construct
    CATLASS_DEVICE
    BlockFTGemvCENoSplitKPreloadFI(Arch::ResourceAIV<ArchTag> &resource, uint32_t UBufAddrStart = 0)
    {
        uint32_t UbCOffset = UBufAddrStart;
        uint32_t UbYOffset = UBufAddrStart + Cbuf_SIZE_;
        uint32_t UbABeOffset = UBufAddrStart + Cbuf_SIZE_ + Ybuf_SIZE_;
    ...
    ~BlockFTGemvCENoSplitKPreloadFI()
    {
        for (uint32_t i = 0; i < STAGES; i++) {
            ...
            AscendC::WaitFlag<AscendC::HardEvent::S_MTE2>(UbFICEventList[i]);
        }
    ...
    void operator()(
        ...
        uint32_t inject_num_blk_actual,
        uint32_t inject_num_blk_align,
        uint32_t inject_row_interval,
        uint32_t inject_col,
        uint32_t inject_bit,
        int32_t block_ce_index,
        GM_ADDR success_ptr,
        AscendC::GlobalTensor<int32_t> const &gmFIIndex,
        AscendC::GlobalTensor<ElementA> const &gmFIFlipData,
        LayoutY const &layoutFIInfo,
        Catlass::Arch::CrossCoreFlagWithReverse<> & flagAicFinishStore)
    {
        ...
        m_actual_part_next = m_actual_total_next / aiv_part_num;


        uint32_t M_start_offset = AscendC::GetSubBlockIdx() * m_actual_part;
        uint32_t M_start_offset_next = AscendC::GetSubBlockIdx() * m_actual_part_next;
        ...
        if(AscendC::GetSubBlockIdx() == (aiv_part_num -1)) {
            ...
            m_actual_part_next = m_actual_total_next - M_start_offset_next;
        }
        ...
        uint32_t inject_row_start_raw = inject_row_start;
        uint32_t inject_row_end_raw = inject_row_start + (inject_num_blk_actual-1) * inject_row_interval;

        SecondRemainFI = 0;

        if(inject_row_end_raw < m_actual_total){

            BlockFIRowStart = inject_row_start_raw;
            BlockFIRowEnd = inject_row_end_raw;
            SecondRemainFI = 0;

        } else {
            uint32_t first_injected_num = ((m_actual_total - 1 - inject_row_start_raw) / inject_row_interval) + 1;

            if(first_injected_num >= inject_num_blk_actual){
                SecondRemainFI = 0;
                first_injected_num = inject_num_blk_actual;
            }else{
                SecondRemainFI = inject_num_blk_actual - first_injected_num;
            }
            ...
            uint32_t second_injected_start_raw = inject_row_start_raw + first_injected_num * inject_row_interval;

            uint32_t second_injected_start = second_injected_start_raw % m_actual_total;

            BlockFIRowStart = (second_injected_start < inject_row_start_raw) ? second_injected_start : inject_row_start_raw;
            BlockFIRowEnd = inject_row_start_raw + (first_injected_num - 1) * inject_row_interval;
        }

        if(AscendC::GetSubBlockIdx() == (aiv_part_num -1)) {
            TileFIRowEnd = (BlockFIRowEnd < m_actual_total) ? BlockFIRowEnd : m_actual_total - 1;

            if(BlockFIRowEnd <= M_start_offset){
                TileFIRowStart = BlockFIRowEnd;
            }else{
                if(inject_row_start_raw >= M_start_offset){
                    TileFIRowStart = inject_row_start_raw;
                }else{
                    uint32_t can_inject_num = ((M_start_offset - 1 - inject_row_start_raw) / inject_row_interval);
                    TileFIRowStart = inject_row_start_raw + (can_inject_num + 1) * inject_row_interval;
                }
            } 

            uint32_t InjectUnitNum = CeilDiv(M_start_offset, TileMRound);
            TileFIRowOffset = InjectUnitNum * inject_num_blk_align;

        }else{
            TileFIRowStart = (BlockFIRowStart <= m_actual_part) ? BlockFIRowStart : m_actual_part;
            if(BlockFIRowStart >= m_actual_part){
                TileFIRowEnd = m_actual_part;
            }else{
                if(inject_row_start_raw >= m_actual_part){
                    uint32_t aim_inject_num = (SecondRemainFI > 0) ? SecondRemainFI - 1 : 0;
                    uint32_t can_inject_num = ((m_actual_part - 1 - TileFIRowStart) / inject_row_interval);

                    if(can_inject_num >= aim_inject_num){
                        can_inject_num = aim_inject_num;
                    }

                    TileFIRowEnd = TileFIRowStart + can_inject_num * inject_row_interval;

                }else{
                    if(BlockFIRowEnd < m_actual_part){
                        TileFIRowEnd = BlockFIRowEnd;
                    }else{
                        uint32_t can_inject_num_raw = ((m_actual_part - 1 - inject_row_start_raw) / inject_row_interval) + 1;
                        uint32_t aim_inject_num_total = (SecondRemainFI > 0) ? SecondRemainFI + can_inject_num_raw : can_inject_num_raw;
                        uint32_t aim_inject_num = aim_inject_num_total - 1;
                        aim_inject_num = (aim_inject_num < inject_num_blk_actual) ? aim_inject_num : inject_num_blk_actual - 1;
        ...
        if(isFirstBlock){
            ...
            auto UbFIIndexTensor = UbFIIndexTensorList[UbOutListId];
            auto UbFIFlipDataTensor = UbFIFlipDataTensorList[UbOutListId];

            AscendC::Duplicate<ElementY>(UbYTensor, (ElementY)0.0, m_actual);
            AscendC::Duplicate<ElementY>(UbThreTensor, (ElementY)0.0, m_actual);
            AscendC::Duplicate<int32_t>(UbFIIndexTensor, (int32_t)(-1), inject_num_blk_align);
            AscendC::Duplicate<ElementA>(UbFIFlipDataTensor, (ElementA)(0.0), inject_num_blk_align);
            

            AscendC::PipeBarrier<PIPE_V>();
            AscendC::SetFlag<AscendC::HardEvent::V_V>((event_t)(UbOutZEventList[UbOutListId]));
            AscendC::SetFlag<AscendC::HardEvent::V_S>((event_t)(UbOutZEventList[UbOutListId]));
        ...
        for(uint32_t mLoopIdx = 0; mLoopIdx < Mloop; mLoopIdx++){
            ...
            int32_t CE_block_offset = block_ce_index + (int32_t)mLoopOffset;

            uint32_t blk_inject_offset = (mLoopIdx * inject_num_blk_align) + TileFIRowOffset;
            ...
            if (mLoopIdx < (Mloop -1)) {
                ...
                auto UbFIIndexTensorNext = UbFIIndexTensorList[UbOutListIdNext];
                auto UbFIFlipDataTensorNext = UbFIFlipDataTensorList[UbOutListIdNext];
                ...
                AscendC::Duplicate<int32_t>(UbFIIndexTensorNext, (int32_t)(-1), inject_num_blk_align);
                AscendC::Duplicate<ElementA>(UbFIFlipDataTensorNext, (ElementA)(0.0), inject_num_blk_align);

                AscendC::PipeBarrier<PIPE_V>();

                AscendC::SetFlag<AscendC::HardEvent::V_V>((event_t)(UbOutZEventList[UbOutListIdNext]));
                AscendC::SetFlag<AscendC::HardEvent::V_S>((event_t)(UbOutZEventList[UbOutListIdNext]));
            ...
            }else if(mLoopIdx == (Mloop -1) && hasNextBlock){
                uint32_t UbOutListIdNext = ((UbOutListId + 1) < STAGES) ? (UbOutListId + 1) : 0;
                ...
                auto UbAMinTensorNext = UbAMinTensorforYList[UbOutListIdNext];
                vecCopyGmToUbInY(UbAMinTensorNext, gmNextAMin[mLoopOffsetNext], m_actual_next);

                auto UbAMeanTensorNext = UbAMeanTensorList[UbOutListIdNext];
                vecCopyGmToUbInY(UbAMeanTensorNext, gmNextAMean[mLoopOffsetNext], m_actual_next);

                auto UbABeTensorNext = UbABeTensorList[UbOutListIdNext];
                vecCopyGmToUbInY(UbABeTensorNext, gmNextABe[mLoopOffsetNext], m_actual_next);

                auto UbAMaxTensorNext = UbAMaxTensorforYList[UbOutListIdNext];
                vecCopyGmToUbInY(UbAMaxTensorNext, gmNextAMax[mLoopOffsetNext], m_actual_next);
            ...
            */
            ...
            AscendC::WaitFlag<AscendC::HardEvent::S_MTE2>((event_t)(UbFICEventList[UbInListId]));
            ...
            AscendC::SetFlag<AscendC::HardEvent::MTE2_S>((event_t)(UbFICEventList[UbInListId]));

            AscendC::WaitFlag<AscendC::HardEvent::V_S>((event_t)(UbOutZEventList[UbOutListId]));
            ...
            for (uint32_t nLoopIdx = 0; nLoopIdx < NloopV; nLoopIdx++) {
                ...
                if (nLoopIdx < NloopV - 1) {
                    ...
                    AscendC::WaitFlag<AscendC::HardEvent::S_MTE2>((event_t)(UbFICEventList[UbInListIdNext]));
                    auto layoutCInUb = layoutC.GetTileLayout(MakeCoord(TileMRound, TileNRound));
                    ...
                    AscendC::SetFlag<AscendC::HardEvent::MTE2_S>((event_t)(UbFICEventList[UbInListIdNext]));
                }

                AscendC::WaitFlag<AscendC::HardEvent::MTE2_S>((event_t)(UbFICEventList[UbInListId]));

                uint32_t tileMStart = mLoopOffset;
                uint32_t tileMEnd = tileMStart + m_actual;
                uint32_t tileNStart = TileNRound * nLoopIdx;
                uint32_t tileNEnd = tileNStart + n_actual;

                if (success_ptr != nullptr && (tileNStart <= inject_col) && (tileNEnd > inject_col) && (tileMStart <= TileFIRowEnd) && (tileMEnd > TileFIRowStart)){

                    for(uint32_t inject_row = TileFIRowStart; inject_row <= TileFIRowEnd; inject_row += inject_row_interval){
                        if ((tileMStart <= inject_row) && (tileMEnd > inject_row)){
                            uint32_t local_row = inject_row - tileMStart;
                            uint32_t local_col = inject_col - tileNStart;
                            uint32_t FIOffset   = local_row * TileNRound + local_col;

                            int32_t FI_Index = (int32_t)CE_block_offset + (int32_t)local_row;

                            uint32_t local_inject_offset = (inject_row  - TileFIRowStart) / inject_row_interval;
                            // uint32_t blk_inject_offset = (uint32_t)local_inject_offset + TileFIRowOffset;
                            auto tileCTensor = UbCTensorList[UbInListId];

                            ElementA currentValue   = tileCTensor.GetValue(FIOffset);
                            // ElementA originalValue  = currentValue;
                            uint32_t dummyFilp      = *reinterpret_cast<uint32_t*>(&currentValue);
                            uint32_t mask           = 1u << inject_bit;

                            if ((mask & dummyFilp) == 0){
                                dummyFilp ^= mask;
                                currentValue        = *reinterpret_cast<ElementA*>(&dummyFilp);
                                tileCTensor.SetValue(FIOffset, currentValue);
                                // __aicore__ inline void SetValue(const uint64_t offset, PrimType value)
                                UbFIIndexTensor.SetValue(local_inject_offset, FI_Index);
                                UbFIFlipDataTensor.SetValue(local_inject_offset, currentValue);
                                *success_ptr        = 1 | *success_ptr;

                            } else {
                                // *success_ptr        = 0;
                            }
                        }else if(inject_row > tileMEnd){
                            break;
                        }else{
                            // do nothing
                        }
                ...
                AscendC::SetFlag<AscendC::HardEvent::S_V>((event_t)(UbFICEventList[UbInListId]));
                AscendC::WaitFlag<AscendC::HardEvent::S_V>((event_t)(UbFICEventList[UbInListId]));
                ...
                AscendC::SetFlag<AscendC::HardEvent::S_MTE2>((event_t)(UbFICEventList[UbInListId]));
                UbInListId = UbInListIdNext;
            }

            AscendC::PipeBarrier<PIPE_V>();

            AscendC::SetFlag<AscendC::HardEvent::S_MTE3>((event_t)(UbOutZEventList[UbOutListId]));
            ...
            */
            ...
            AscendC::WaitFlag<AscendC::HardEvent::S_MTE3>((event_t)(UbOutZEventList[UbOutListId]));

            auto layoutDstFIInfo = layoutFIInfo.GetTileLayout(TensorCoord(inject_num_blk_align));
            auto layoutComputeFIInfoInUb = layoutFIInfo.GetTileLayout(TensorCoord(inject_num_blk_align));

            vecCopyUbToGmforFIIndex(
                gmFIIndex[blk_inject_offset], 
                UbFIIndexTensorList[UbOutListId], 
                layoutDstFIInfo, 
                layoutComputeFIInfoInUb);

            vecCopyUbToGmforFIData(
                gmFIFlipData[blk_inject_offset], 
                UbFIFlipDataTensorList[UbOutListId], 
                layoutDstFIInfo, 
                layoutComputeFIInfoInUb);
...
protected:
    ...
    AscendC::LocalTensor<int32_t> UbFIIndexTensorList[STAGES]; // 用于存储注入故障的索引
    AscendC::LocalTensor<ElementA> UbFIFlipDataTensorList[STAGES]; // 用于存储注入故障后的数据
    ...
    int32_t UbFICEventList[STAGES]; // 用于注入故障的事件列表
    ...
    uint32_t BlockFIRowStart, BlockFIRowEnd;
    uint32_t TileFIRowStart, TileFIRowEnd;
    uint32_t TileFIRowOffset;
    ...
    VecCopyUbToGmforFIIndex vecCopyUbToGmforFIIndex; // 用来输出注入故障的索引
    VecCopyUbToGmforFIData vecCopyUbToGmforFIData; // 用来输出注入故障的数据
