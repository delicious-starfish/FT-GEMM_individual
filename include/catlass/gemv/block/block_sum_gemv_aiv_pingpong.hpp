...
public:
    ...
    CATLASS_DEVICE
    BlockSumGemvPingPong() {}

    /// Construct
    CATLASS_DEVICE
    BlockSumGemvPingPong(Arch::Resource<ArchTag> &resource, uint32_t UBufAddrStart = 0)
    {
    ...
    void operator()(
        ...
        GemvCoord const &actualShape, Gemv::helper::FT_ENC_TYPE enc_type)
    {
        /*
        ...
        */
        ...
        if (enc_type == Gemv::helper::FT_ENC_TYPE::BE){
            TileMRound = RoundUp(UBTileShape::K, UBAlignHelper::ALIGN);
            TileNRound = RoundUp(UBTileShape::N, UBAlignHelper::ALIGN);
        } else if(enc_type == Gemv::helper::FT_ENC_TYPE::ABE){
            TileMRound = RoundUp(UBTileShape::M, UBAlignHelper::ALIGN);
            TileNRound = RoundUp(UBTileShape::K, UBAlignHelper::ALIGN);   
        } else if(enc_type == Gemv::helper::FT_ENC_TYPE::CE){
            TileMRound = RoundUp(UBTileShape::M, UBAlignHelper::ALIGN);
            TileNRound = RoundUp(UBTileShape::N, UBAlignHelper::ALIGN);
        } else if(enc_type == Gemv::helper::FT_ENC_TYPE::ETA){
            TileMRound = RoundUp(UBTileShape::K, UBAlignHelper::ALIGN);
            TileNRound = RoundUp(UBTileShape::M, UBAlignHelper::ALIGN);
        } else if(enc_type == Gemv::helper::FT_ENC_TYPE::ETAB){
            TileMRound = RoundUp(UBTileShape::N, UBAlignHelper::ALIGN);
            TileNRound = RoundUp(UBTileShape::K, UBAlignHelper::ALIGN);
        } else{
            TileMRound = RoundUp(UBTileShape::N, UBAlignHelper::ALIGN);
            TileNRound = RoundUp(UBTileShape::M, UBAlignHelper::ALIGN);
