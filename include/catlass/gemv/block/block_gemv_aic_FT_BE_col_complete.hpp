#ifndef CATLASS_GEMV_BLOCK_BLOCK_AIC_FT_BE_COLCOMPLETE_HPP
#define CATLASS_GEMV_BLOCK_BLOCK_AIC_FT_BE_COLCOMPLETE_HPP
...
// Gemv::helper::FT_AIC_BE_SCHEME  BeScheme_,nnn
...
struct BlockFTGemvBe<
    ...
    Gemv::helper::FT_AIC_BE_SCHEME::COLCOMPLETE,
    UBBlockShape_,
    L1TileShape_,
    L0TileShape_,
    AType_,
...
public:
    ...
    static_assert(L0TileShape::N == L1TileShape::N,
        ...
        bool isFirstBlock, bool hasNextBlock)
    {
        ...
        for (uint32_t nLoopIdx = 0; nLoopIdx < nTileCount; nLoopIdx++) {
            ...
            for(uint32_t mLoopIdx=0; mLoopIdx < mTileCount; mLoopIdx++){
                ...
                uint32_t mRound = RoundUp<L1XAlignHelper::M_ALIGNED>(m_actual_now);
                uint32_t mPartLoop = CeilDiv<L0TileShape::M>(m_actual_now);
                
                AscendC::WaitFlag<AscendC::HardEvent::FIX_M>(l0CEventList[l1ListId]);
                auto l0CTensor = l0CTensorList[l1ListId];

                uint32_t nPartIdx = 0;
                // Locate the current tile on L0A
                auto l0ATile = l0ATensorList[l1ListId];
                LayoutXInL0 layoutxInL0 =
                        LayoutXInL0::template MakeLayout<ElementX>(L1XAlignHelper::M_ALIGNED, L0TileShape::N);

                MatrixCoord l1xOffset{0, nPartIdx * L0TileShape::N};
                auto l1ATile = l1ATensor[layoutXInL1.GetOffset(l1xOffset)];
                    
                AscendC::WaitFlag<AscendC::HardEvent::M_MTE1>(l0AEventList[l1ListId]);
                // Load current tile from L1 to L0A
                copyL1ToL0A(l0ATile, l1ATile, layoutxInL0, layoutXInL1);
                AscendC::SetFlag<AscendC::HardEvent::MTE1_M>(l0AEventList[l1ListId]);

                AscendC::WaitFlag<AscendC::HardEvent::MTE1_M>(l0AEventList[l1ListId]);
                for (uint32_t mPartIdx = 0; mPartIdx < mPartLoop; mPartIdx++) {
                    uint32_t mPartActual =
                        (mPartIdx < mPartLoop - 1) ? L0TileShape::M : (m_actual_now - mPartIdx * L0TileShape::M);

                    // Locate the current tile on L0B
                    auto l0BTile = l0BTensorList[l0BListId];
                    LayoutAInL0 layoutAInL0 = LayoutAInL0::template MakeLayout<ElementA>(mPartActual, L0TileShape::N);

                    MatrixCoord l1AOffset{mPartIdx * L0TileShape::M, nPartIdx * L0TileShape::N};
                    ...
                    MatrixCoord l0COffset{0, mPartIdx * L0TileShape::M};

                    auto l0CTile = l0CTensor[layoutInL0C.GetOffset(l0COffset)];
                    // L0TileShape::M
                    AscendC::WaitFlag<AscendC::HardEvent::MTE1_M>(l0BEventList[l0BListId]);
                    
                    tileMmad(l0CTile, l0ATile, l0BTile, L1XAlignHelper::M_ALIGNED, mPartActual, L0TileShape::N, initC);
                ...
                AscendC::SetFlag<AscendC::HardEvent::M_MTE1>(l0AEventList[l1ListId]);
                ...
                // m_actual = m_actual_next;
                // auto l0CTile = l0CTensor[(singleIdx % L0C_TILE_NUM) * L0C_TILE_SIZE];
                // copy block out
                // m_actual_now = (mLoopIdx < (mTileCount - 1)) ? TileMRound : m_actual_total - mLoopIdx * TileMRound;
                ...
                copyL0CToGm(gmBlockY[layoutY.GetOffset(gmTileYOffset)], l0CTensor, layoutBlock, layoutInL0C);
