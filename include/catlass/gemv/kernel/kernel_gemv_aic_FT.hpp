#ifndef CATLASS_GEMV_KERNLE_GEMV_AIC_FT_HPP
#define CATLASS_GEMV_KERNLE_GEMV_AIC_FT_HPP
...
public:
    ...
    struct Params {
        ...
        Params(GemvCoord const &problemShape_, GM_ADDR ptrX_, LayoutX layoutX_, GM_ADDR ptrA_, LayoutA layoutA_,
            GM_ADDR ptrWorkspace_, EpilogueParams const &epilogueParams_, FT_ENC_TYPE enc_type_, GM_ADDR ptrZ_)
            : problemShape(problemShape_), ptrX(ptrX_), layoutX(layoutX_), ptrA(ptrA_), layoutA(layoutA_),
              ptrWorkspace(ptrWorkspace_), epilogueParams(epilogueParams_), enc_type(enc_type_), ptrZ(ptrZ_) {}
    };
    ...
    static Params ToUnderlyingArguments(const Arguments &args, uint8_t *workspace)
    {
        ...
        if(args.enc_type == FT_ENC_TYPE::ETC){
            ...
            Params params{problemShape, args.ptrX, layoutX, args.ptrA, layoutA, workspace, epilogueParams,args.enc_type, args.ptrZ};
            return params;
        }else{
            LayoutX layoutX{n};
            LayoutZ layoutZ{m};
    ...
    void ETC_op(Params const& params)
    {
        ...
        // ptrWorkspace
        AscendC::GlobalTensor<ElementY> gmY;
        gmY.SetGlobalBuffer((__gm__ ElementY *)params.ptrZ);

        layout::RowMajor layoutY(1, params.problemShape.n());
        ...
        blockGemv.FillScalarInLocal(AscendC::GetBlockIdx(), static_cast<ElementX>(1.0),FT_ENC_TYPE::ETC);
        AscendC::PipeBarrier<PIPE_ALL>();

        for (uint32_t loopIdx = AscendC::GetBlockIdx(); loopIdx < coreLoops; loopIdx += AscendC::GetBlockNum()) {
            // Compute Block location
            uint32_t NGmBlockIdx = loopIdx;
            uint32_t NGmActual = (NGmBlockIdx == NLoops - 1) ? (N - NGmBlockIdx * maxNPerBlock) : maxNPerBlock;
            uint32_t MGmActual = M;
            // int64_t gmOffsetX;
            int64_t gmOffsetA;
            int64_t gmOffsetY;
            int64_t gmOffsetNextA;
            ...
            if constexpr (std::is_same<LayoutA, Catlass::layout::RowMajor>::value) {
                // gmOffsetX = 0;
                gmOffsetA = NGmBlockIdx * maxNPerBlock;

                gmOffsetY = NGmBlockIdx * maxNPerBlock;
            } else {
                // gmOffsetX = 0;
                gmOffsetA = NGmBlockIdx * maxNPerBlock * params.layoutA.stride(1);
            ...
            bool isFirstBlock = (loopIdx == AscendC::GetBlockIdx());
            bool hasNextBlock = false;
            uint32_t NNextGmBlockIdx;
            GemvCoord nextActualBlockShape;
            if (loopIdx + AscendC::GetBlockNum() < coreLoops) {
                hasNextBlock = true;
                uint32_t nextLoopIdx = loopIdx + AscendC::GetBlockNum();
                NNextGmBlockIdx = nextLoopIdx;
                uint32_t NNextGmActual =
                    (NNextGmBlockIdx == NLoops - 1) ? (N - NNextGmBlockIdx * maxNPerBlock) : maxNPerBlock;
                uint32_t MNextGmActual = M;
                nextActualBlockShape = GemvCoord(MNextGmActual, NNextGmActual);
            }

            if constexpr (std::is_same<LayoutA, Catlass::layout::RowMajor>::value) {
                
                gmOffsetNextA = NNextGmBlockIdx * maxNPerBlock;

                gmOffsetNextY = NNextGmBlockIdx * maxNPerBlock;
            } else {
                // gmOffsetNextX = 0;
                gmOffsetNextA = NNextGmBlockIdx * maxNPerBlock  * params.layoutA.stride(1); 
                gmOffsetNextY = NNextGmBlockIdx * maxNPerBlock;
            }
    ...
    void EpilogueETC(Params const& params)
    {
        ...
        gmZ.SetGlobalBuffer((__gm__ ElementZ *)params.ptrZ);

        layout::VectorLayout layoutY(params.problemShape.n());
        ...
        // Loop through the epilogue calculations of each basic block
        layout::VectorLayout::TensorCoord blockShape{L1TileShape::N};

        for (uint32_t loopIdx = aicoreIndex; loopIdx < coreLoops; loopIdx += aicoreNum) {
            // Compute block location
            layout::VectorLayout::TensorCoord blockCoord{loopIdx};
            uint32_t NGmActual = (loopIdx == coreLoops) ? N - loopIdx * maxNPerBlock : maxNPerBlock;

            layout::VectorLayout::TensorCoord actualBlockShape{NGmActual};

            // Get the offset
            layout::VectorLayout::TensorCoord blockOffset = blockCoord * blockShape;

            int64_t gmOffSetZ = loopIdx * L1TileShape::N;

            // Get the data and layout of y under the current basic block
            auto gmBlockY = gmY[layoutY.GetOffset(blockOffset)];
            auto gmBlockZ = gmZ[gmOffSetZ];
            // auto layoutBlockY = layoutY.GetTileLayout(actualBlockShape);
