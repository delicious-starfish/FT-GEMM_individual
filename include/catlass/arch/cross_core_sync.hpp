...
// constexpr FlagID AIV_INTER_BLOCK_BARRIER = 4;
// constexpr FlagID AIC_INTER_BLOCK_BARRIER = 5;
// constexpr FlagID AIV_INTER_SUBBLOCK_BARRIER = 6;
...
{
    constexpr FlagID flagId = BarrierFlag<MODE, AscendC::AIC>::ID;
    AscendC::CrossCoreSetFlag<MODE, PIPE>(flagId);
    AscendC::CrossCoreWaitFlag(flagId);
}
...
{
    constexpr FlagID flagId = BarrierFlag<MODE, AscendC::AIV>::ID;
    AscendC::CrossCoreSetFlag<MODE, PIPE>(flagId);
    AscendC::CrossCoreWaitFlag(flagId);
}
...
{
    /*
    同一flagId的计数器最多设置16次,即同一个类别同一个位置上的同一个ID最多连续调用SetFlag 16 次，
    将该ID对应计数器的值累加到15 
    */
    AscendC::CrossCoreSetFlag<MODE, PIPE>(flag.id);
