...
>
class MatmulFaultTolerancePingPong {
public:
    ...
    using BlockEncodeScheduler = BlockEncodeScheduler_;
    ...
    void operator()<AscendC::AIV>(Params const &params)
    {
        ...
        */
        ...
        GemvCoord BeproblemShape{params.problemShape.k(),params.problemShape.n()};
        BlockScheduler matmulBlockScheduler(params.problemShape, MakeCoord(L1TileShape::M, L1TileShape::N));
        BlockEncodeScheduler gemvEncBlockScheduler(BeproblemShape,MakeCoord(UBTileShape::M, UBTileShape::K));

        uint32_t coreLoops = matmulBlockScheduler.GetCoreLoops();
        uint32_t becoreLoops = gemvEncBlockScheduler.GetCoreLoops();
        ...
        MatrixCoord loopsMNBe = gemvEncBlockScheduler.loopsMN;
        ...
        if(params.checkSumType == CheckSumType::ROW_CHECKSUM) {
            ...
            // 先做 Be
            // loopsMN.column()
            for(uint32_t loopIdx = aivIndex; loopIdx < becoreLoops; loopIdx += aivNum) {
                // 每次输入的列数为L1Tile的shape

                GemvCoord BeblockCoord = gemvEncBlockScheduler.GetBlockCoord(loopIdx);
                GemvCoord BeactualBlockShape = gemvEncBlockScheduler.GetActualBlockShape(BeblockCoord);

                // Compute initial location in logical coordinates
            
                MatrixCoord offsetB{BeblockCoord.m() * UBTileKRound, BeblockCoord.n() * UBTileNRound};

                int64_t gmOffsetBE = BeblockCoord.n() * UBKRound + BeblockCoord.m() * UBTileKRound;
                int64_t gmOffsetB = params.layoutB.GetOffset(offsetB);

                layout::VectorLayout layoutBE{BeactualBlockShape.m()};
                blockSumGemv.rowSum(gmB[gmOffsetB], params.layoutB, 
                    gmD[gmOffsetBE], layoutBE, BeactualBlockShape, Gemv::helper::FT_ENC_TYPE::BE);
            ...
            if(aivCore == AivCore::AIV0) {
                ...
                for (uint32_t loopIdx = aicoreIndex; loopIdx < coreLoops; loopIdx += aicoreNum) {
                    ...
                    for(uint32_t kLoopIdx = 0; kLoopIdx < kTileCount; kLoopIdx++){
                        ...
                        // 计算对应的ksplit 暂存中间结果：
                        blockSumGemv.rowSum(gmWorkspace[gmCSplitKBufferIdx * L1TileShape::M * L1TileShape::N], layoutCSplitKInGm, 
                            gmD[gmOffsetCE], layoutCE, gemvActualBlockShape, Gemv::helper::FT_ENC_TYPE::CE);
                ...
                for(uint32_t loopIdx = aicoreIndex; loopIdx < coreLoops; loopIdx += aicoreNum) {
                    ...
                    for (uint32_t kLoopIdx = 0; kLoopIdx < kTileCount; kLoopIdx++) {
                        ...
                        Arch::CrossCoreWaitFlagWithReverse<0x2, PIPE_MTE3>(flagAicFinishStore);
                        blockSumGemv(gmA[gmOffsetA], params.layoutA, gmD[gmoffsetBE], layoutBE, 
                            gmD[gmoffsetABE], layoutABE, gemvActualBlockShape,  Gemv::helper::FT_ENC_TYPE::ABE);
                    }
                }
            }
        }
