...
#ifndef CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_FAULT_HPP
#define CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_FAULT_HPP
...
public:
    ...
    /// Construct
    CATLASS_DEVICE
    BlockMmadFault(Arch::Resource<ArchTag> &resource, uint32_t l1BufAddrStart = 0)
    {
        ...
        for (uint32_t i = 0; i < STAGES; i++) {
            ...
            l0CTensorList[i] = resource.l0CBuf.template GetBufferByByte<ElementC>(L0C_PINGPONG_BUF_SIZE * i);
    ...
    void operator()(
        ...
        AscendC::GlobalTensor<ElementC> const &gmWorkspace, uint32_t &gmCSplitKBufferIdx,
        GemmCoord const &actualShape, Arch::CrossCoreFlagWithReverse<> &flagAicFinishStore)
    {
        ...
        for (uint32_t kLoopIdx = 0; kLoopIdx < kTileCount; kLoopIdx++) {
            ...
            for (int mPartIdx = 0; mPartIdx < mPartLoop; mPartIdx++) {
                ...
                for (int kPartIdx = 0; kPartIdx < kPartLoop; kPartIdx++) {
                    ...
                    for (int nPartIdx = 0; nPartIdx < nPartLoop; nPartIdx++) {
                        ...
                        bool initCSplitK = (kPartIdx == 0);
                        uint8_t unitFlagSplitK = 0b00;
                        ...
                        tileMmad(l0CTile, l0ATile, l0BTile, mPartActual, nPartActual, kPartActual, initCSplitK, unitFlagSplitK);
            ...
            // copy block out
            /*
            与典型的矩阵乘法的主要修改其实就在这里，每次输出放在16个GM中间存储的位置之一
            */
            if constexpr (!ENABLE_UNIT_FLAG) {
                // 通知已经运算完可以写到GM了
                ...
                // 每次在一个L1 Tile 上做校验，所以一个向后offset一个L1 Tile的空间
                copyL0CToGm(gmWorkspace[gmCSplitKBufferIdx * L1TileShape::M * L1TileShape::N], l0CTensor, layoutCSplitKInGm, layoutInL0C);
                AscendC::SetFlag<AscendC::HardEvent::FIX_M>(l0CEventList[l1ListId]);
            } else {
                copyL0CToGm(gmWorkspace[gmCSplitKBufferIdx * L1TileShape::M * L1TileShape::N], l0CTensor, layoutCSplitKInGm, layoutInL0C, 0b11);
            }

            // Synchronize cross core
            /*
            与该AIC Cube core 对应的两个AIV core进行同步，设定AIC计算上L1 Tile 计算完成了，可以进行校验了。
            */
            ...
            kActual = kActualNext;
            /*
            确定下一次输出的位置，这里因为 连续setFlag 16次后，等待一次同步翻转，
            即WaitFlag到校验或者GEMV结束，即校验同步一次
            所以输出的中间位置即为0~15 index 循环
            因为一个FlagId 最多连续set 16 次，对应中间存储空间内16个 L1 Tile的局部存储位置
            对应 Index 0~15
            */
...
protected:
    ...
    AscendC::LocalTensor<ElementC> l0CTensorList[STAGES];
...
#endif // CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_FAULT_HPP
