...
#ifndef CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_FAULT_HPP_BAK
#define CATLASS_GEMM_BLOCK_BLOCK_MMAD_PINGPONG_FAULT_HPP_BAK
...
public:
    ...
    BlockMmadFault(Arch::Resource<ArchTag> &resource, uint32_t l1BufAddrStart = 0)
    {
        ...
        l0CSplitKTensor = resource.l0CBuf.template GetBufferByByte<ElementAccumulator>(L0C_SIZE / 2);
        ...
        GemmCoord const &actualShape, Arch::CrossCoreFlagWithReverse<> &flagAicFinishStore)
    {
        ...
        for (uint32_t kLoopIdx = 0; kLoopIdx < kTileCount; kLoopIdx++) {
            ...
            for (int mPartIdx = 0; mPartIdx < mPartLoop; mPartIdx++) {
                ...
                for (int kPartIdx = 0; kPartIdx < kPartLoop; kPartIdx++) {
                    ...
                    for (int nPartIdx = 0; nPartIdx < nPartLoop; nPartIdx++) {
                        ...
                        auto l0CSplitKTile = l0CSplitKTensor[layoutInL0C.GetOffset(l0COffset)];
                        ...
                        // Perform calculation operations
                        tileMmad(l0CSplitKTile, l0ATile, l0BTile, mPartActual, nPartActual, kPartActual, initCSplitK, unitFlagSplitK);
            ...
            if constexpr (!ENABLE_UNIT_FLAG) {
                ...
                copyL0CToGm(gmWorkspace[gmCSplitKBufferIdx * L1TileShape::M * L1TileShape::N], l0CSplitKTensor, layoutCSplitKInGm, layoutInL0C);
                AscendC::SetFlag<AscendC::HardEvent::FIX_M>(EVENT_ID0);
                AscendC::WaitFlag<AscendC::HardEvent::FIX_M>(EVENT_ID0);
            } else {
                copyL0CToGm(gmWorkspace[gmCSplitKBufferIdx * L1TileShape::M * L1TileShape::N], l0CSplitKTensor, layoutCSplitKInGm, layoutInL0C, 0b11);
            }
...
protected:
    ...
    AscendC::LocalTensor<ElementAccumulator> l0CSplitKTensor;
