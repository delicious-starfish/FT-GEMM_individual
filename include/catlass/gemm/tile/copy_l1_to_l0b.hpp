...
/// new add gemm
template<class ArchTag, class Element>
struct CopyL1ToL0B<ArchTag, Catlass::Gemm::GemmType<Element, layout::zZ, AscendC::TPosition::A1>, Catlass::Gemm::GemmType<Element, layout::nZ, AscendC::TPosition::B2>>{
    using LayoutDst = layout::nZ;
    using LayoutSrc = layout::zZ;
    ...
    void operator()(
        ...
        参数名称                        含义
        startIndex      分形矩阵ID，说明搬运起始位置为源操作数中第几个分形（0为源操作数中第1个分形矩阵）。
                        取值范围：startIndex∈[0, 65535] 。单位：512B。默认为0。
        
        repeatTimes     迭代次数，每个迭代可以处理512B数据。取值范围：repeatTimes∈[1, 255]。
                        即每次处理的源分形的数量：在这里每次处理一个分块行的数据，即源矩阵一组行上所有的分块
        
        srcStride       相邻迭代间，源操作数前一个分形与后一个分形起始地址的间隔，单位：512B。
                        取值范围：src_stride∈[0, 65535]。默认为0。
        
        sid             预留参数，配置为0即可。
        
        dstGap          相邻迭代（repeat）间，目的操作数前一个分形结束地址与后一个分形起始地址的间隔，
                        单位：512B。取值范围：dstGap∈[0, 65535]。默认为0。
                        注： Atlas 训练系列产品 此参数不使能。

        ifTranspose     是否启用转置功能，对每个分形矩阵进行转置，默认为false:
                            true：启用
                            false：不启用
                        注意：只有A1->A2和B1->B2通路才能使能转置，使能转置功能时，
                        源操作数、目的操作数仅支持uint16_t/int16_t/half数据类型。
...
template<class ArchTag>
struct CopyL1ToL0B<ArchTag, Catlass::Gemm::GemmType<float, layout::zZ, AscendC::TPosition::A1>, Catlass::Gemm::GemmType<float, layout::nZ, AscendC::TPosition::B2>>{
    using Element = float;
    using LayoutDst = layout::nZ;
    using LayoutSrc = layout::zZ;
...
template<class ArchTag>
struct CopyL1ToL0B<ArchTag, Catlass::Gemm::GemmType<int8_t, layout::zN, AscendC::TPosition::A1>, Catlass::Gemm::GemmType<int8_t, layout::nZ, AscendC::TPosition::B2>>{
    using Element = int8_t;
    using LayoutDst = layout::nZ;
    using LayoutSrc = layout::zN;
...
template <class ArchTag, class Element>
struct CopyL1ToL0B<ArchTag, Catlass::Gemm::GemmType<Element, layout::nZ, AscendC::TPosition::A1>, Catlass::Gemm::GemmType<Element, layout::nZ, AscendC::TPosition::B2>> {
    using LayoutDst = layout::nZ;
    using LayoutSrc = layout::nZ;
...
/*
Add the tile layout to support the column checksum encoding
*/
/*
When matrix is Row Major
*/
/// Partial specialization for zN in and nZ out.
template <class ArchTag, class Element>
struct CopyL1ToL0B<ArchTag, Gemm::GemmType<Element, layout::zN, AscendC::TPosition::A1>, Gemm::GemmType<Element, layout::nZ, AscendC::TPosition::B2>> {
    using LayoutDst = layout::nZ;
    using LayoutSrc = layout::zN;
...
// /// Partial specialization for int8_t, zN in and nZ out for GEMV.
// template <class ArchTag>
// struct CopyL1ToL0B<ArchTag, Gemm::GemmType<int8_t, layout::zN, AscendC::TPosition::A1>, Gemm::GemmType<int8_t, layout::nZ, AscendC::TPosition::B2>> {
//     using Element = int8_t;
//     using LayoutDst = layout::nZ;
//     using LayoutSrc = layout::zN;
...
/// Partial specialization for float, zN in and nZ out for GEMV.
template <class ArchTag>
struct CopyL1ToL0B<ArchTag, Gemm::GemmType<float, layout::zN, AscendC::TPosition::A1>, Gemm::GemmType<float, layout::nZ, AscendC::TPosition::B2>> {
    using Element = float;
    using LayoutDst = layout::nZ;
    using LayoutSrc = layout::zN;
...
/// Partial specialization for zN in and nZ out.
template <class ArchTag, class Element>
struct CopyL1ToL0B<ArchTag, Gemm::GemmType<Element, layout::zN, AscendC::TPosition::B1>, Gemm::GemmType<Element, layout::nZ, AscendC::TPosition::B2>> {
    using LayoutDst = layout::nZ;
    using LayoutSrc = layout::zN;
...
// /// Partial specialization for int8_t, zN in and nZ out for GEMV.
// template <class ArchTag>
// struct CopyL1ToL0B<ArchTag, Gemm::GemmType<int8_t, layout::zN, AscendC::TPosition::B1>, Gemm::GemmType<int8_t, layout::nZ, AscendC::TPosition::B2>> {
//     using Element = int8_t;
//     using LayoutDst = layout::nZ;
//     using LayoutSrc = layout::zN;
...
/// Partial specialization for float, zN in and nZ out for GEMV.
template <class ArchTag>
struct CopyL1ToL0B<ArchTag, Gemm::GemmType<float, layout::zN, AscendC::TPosition::B1>, Gemm::GemmType<float, layout::nZ, AscendC::TPosition::B2>> {
    using Element = float;
    using LayoutDst = layout::nZ;
    using LayoutSrc = layout::zN;
...
/*
When matrix is column major
*/

template<class ArchTag, class Element>
struct CopyL1ToL0A<ArchTag, Catlass::Gemm::GemmType<Element, layout::nN, AscendC::TPosition::A1>, Catlass::Gemm::GemmType<Element, layout::nZ, AscendC::TPosition::A2>>{
    using LayoutDst = layout::nZ;
...
template<class ArchTag, class Element>
struct CopyL1ToL0A<ArchTag, Catlass::Gemm::GemmType<Element, layout::nN, AscendC::TPosition::B1>, Catlass::Gemm::GemmType<Element, layout::nZ, AscendC::TPosition::A2>>{
    using LayoutDst = layout::nZ;
    using LayoutSrc = layout::nN;
...
template <class ArchTag, class Element>
struct CopyL1ToL0B<ArchTag, Catlass::Gemm::GemmType<Element, layout::zN, AscendC::TPosition::A1>, Catlass::Gemm::GemmType<Element, layout::zN, AscendC::TPosition::B2>>{
    using LayoutDst = layout::zN;
    using LayoutSrc = layout::zN;
...
template <class ArchTag, class Element>
struct CopyL1ToL0B<ArchTag, Catlass::Gemm::GemmType<Element, layout::nN, AscendC::TPosition::A1>, Catlass::Gemm::GemmType<Element, layout::zN, AscendC::TPosition::B2>>
{
    using LayoutDst = layout::zN;
    using LayoutSrc = layout::nN;
...
template <class ArchTag>
struct CopyL1ToL0B<ArchTag, Catlass::Gemm::GemmType<float, layout::nN, AscendC::TPosition::A1>, Catlass::Gemm::GemmType<float, layout::zN, AscendC::TPosition::B2>>{
    using LayoutDst = layout::zN;
    using LayoutSrc = layout::nN;
    using Element = float;
...
template <class ArchTag>
struct CopyL1ToL0B<ArchTag, Catlass::Gemm::GemmType<int8_t, layout::nZ, AscendC::TPosition::A1>, Catlass::Gemm::GemmType<int8_t, layout::zN, AscendC::TPosition::B2>>{
    using LayoutDst = layout::zN;
    using LayoutSrc = layout::nZ;
    using Element = int8_t;
